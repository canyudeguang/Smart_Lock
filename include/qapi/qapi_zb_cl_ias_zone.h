...
/** @file qapi_zb_cl_ias_zone.h
   ...
   APIs are provided to create an IAS zone server and client cluster.  Cluster
...
#define QAPI_ZB_CL_IASZONE_ATTR_ID_ZONE_STATUS                                  (0x0002) /**< IAS zone attribute ID for the zone status (read only, map16).  This
                                                                                              attribute is handled internally by the IAS zone server cluster so cannot
                                                                                              be overwritten by the application. */
#define QAPI_ZB_CL_IASZONE_ATTR_ID_IAS_CIE_ADDRESS                              (0x0010) /**< IAS zone attribute ID for the IAS control and indicating equipment address
                                                                                              (EUI64). */
...
/* The following definitions represent the bit mask associated with the zone
   status attribute. */
#define QAPI_ZB_CL_IASZONE_ZONE_STATUS_ALARM1                                   (0x0001) /**< IAS zone status bit mask indicating alarm 1 is active. */
#define QAPI_ZB_CL_IASZONE_ZONE_STATUS_ALARM2                                   (0x0002) /**< IAS zone status bit mask indicating alarm 2 is active. */
#define QAPI_ZB_CL_IASZONE_ZONE_STATUS_TAMPER                                   (0x0004) /**< IAS zone status bit mask indicating a tamper condition. */
#define QAPI_ZB_CL_IASZONE_ZONE_STATUS_BATTERY                                  (0x0008) /**< IAS zone status bit mask indicating a low battery condition. */
#define QAPI_ZB_CL_IASZONE_ZONE_STATUS_SUPERVISION_REPORTS                      (0x0010) /**< IAS zone status bit mask indicating supervision reports are enabled. */
#define QAPI_ZB_CL_IASZONE_ZONE_STATUS_RESTORE_REPORTS                          (0x0020) /**< IAS zone status bit mask indicating restore reports are enabled. */
#define QAPI_ZB_CL_IASZONE_ZONE_STATUS_TROUBLE                                  (0x0040) /**< IAS zone status bit mask indicating a trouble condition. */
#define QAPI_ZB_CL_IASZONE_ZONE_STATUS_AC_MAINS                                 (0x0080) /**< IAS zone status bit mask indicating an AC mains fault. */
#define QAPI_ZB_CL_IASZONE_ZONE_STATUS_TEST                                     (0x0100) /**< IAS zone status bit mask indicating a sensor is in test mode. */
#define QAPI_ZB_CL_IASZONE_ZONE_STATUS_BATTERY_DEFECT                           (0x0200) /**< IAS zone status bit mask indicating a battery is defective. */

/* The following definitions represent the command IDs for the IAS zone client
   cluster. These IDs are provided predominantly for identifying command
   responses. */
...
/**
   Enumeration of events that can be received by a IAS zone server cluster.
*/
typedef enum
{
   ...
   QAPI_ZB_CL_IASZONE_SERVER_EVENT_TYPE_INITIATE_NORMAL_OPERATION_MODE_E, /**< Indicates a initiate normal operation mode event. */
   QAPI_ZB_CL_IASZONE_SERVER_EVENT_TYPE_INITIATE_TEST_MODE_E              /**< Indicates a initiate test mode. */
...
{
   /** The response code of the zone enroll response. */
   qapi_ZB_CL_IASZone_Zone_Enroll_Response_Code_t ResponseCode;

   /** The zone ID of the zone enroll response. */
   uint8_t                                        ZoneID;
...
{
   /** The duration in seconds the IAS zone server shall operate in its
       test mode. */
   uint8_t        TestModeDuration;

   /** The sensitivity level used by the IAS zone server during the test
       mode. */
   uint8_t        CurrentZoneSensitivityLevel;

   /** Result of operation for the response. */
...
/**
   Structure representing the initiate test mode command parameters.
*/
typedef struct qapi_ZB_CL_IASZone_Initiate_Normal_Mode_s
{
   /** Result of operation for the response. */
   qapi_Status_t *Result;
...
{
   /** Current value of Zone status attribute. */
   uint16_t ZoneStatus;

   /** Reserved for additional status info should be set to 0. */
   uint8_t  ExtendedStatus;

   /** Index of the zone in the CIE's zone table. */
   uint8_t  ZoneID;

   /** Amount of time, in quarter-seconds, from the moment of zone status change
       to the successful transmission of Zone status change notification. */
   uint16_t Delay;
...
typedef struct qapi_ZB_CL_IASZone_Zone_Enroll_Request_s
{
   ...
   /** Current value of zone type attribute. */
   qapi_ZB_CL_IASZone_Zone_Type_t                  ZoneType;

   /** Manufacture code held in the node descriptor for the device. */
   uint16_t                                        ManufacturerCode;

   /** Pointer to where the callback should store the status result for the
       enroll response. */
   qapi_Status_t                                  *StatusResult;

   /** Pointer to where the callback should store the response code for the
       enroll response. */
   qapi_ZB_CL_IASZone_Zone_Enroll_Response_Code_t *ResponseCode;

   /** Pointer to where the callback should store the zone ID for the enroll
       response. */
   uint8_t                                        *ZoneID;
} qapi_ZB_CL_IASZone_Zone_Enroll_Request_t;

/**
   Structure representing the data for events received by a IAS zone client
   cluster.
*/
...
typedef struct qapi_ZB_CL_IASZone_Server_Event_Data_s
{
   ...
   union
   {
      ...
      /** Information for zone enroll response. */
      qapi_ZB_CL_IASZone_Zone_Enroll_Response_t Enroll_Response;

      /** Information for the initiate test mode. */
      qapi_ZB_CL_IASZone_Initiate_Test_Mode_t   Intiate_Test_Mode;

      /** Information for the initiate nomal mode. */
      qapi_ZB_CL_IASZone_Initiate_Normal_Mode_t Intiate_Normal_Mode;
   } Data;
...
/**
   ...
   @param[in]  Manufacturer Manufacture code.
   @param[in]  TripToPair   Flag indicating the server supports the trip-to-pair
                            and auto-enroll-request methods for enrollment.  If
                            set to false, the auto-enroll-respnose method is
                            supported.
   @param[in]  Cluster_Info Structure containing the endpoint and attribute
                            information for the cluster.
   @param[in]  Event_CB     Callback function that will handle events for the
...
/**
   @brief This function can be used to populate an attribute list with the
          attributes required for an IAS zone cluster.
