...
typedef struct
{
    ...
    QAPI_WLAN_Coex_Mode_e coex_Mode;                               /**< 3-wire, or PTA mode. */
    QAPI_WLAN_Coex_Num_Priority_Levels_e coex_Num_Priority_Levels; /**< 2 or 4 levels. */
    uint32_t coex_Profile;                                         /**< Narrowband profile in use. */
    uint32_t coex_Antenna_Config;                                  /**< Physical antenna configuration. */
} qapi_WLAN_Coex_Config_Data_t;
...
typedef struct {
    ...
    uint32_t optModeTransitionCnt;
    /**< Count of the times the firmware switches to Opt mode (force awake) to meet
         narrowband coex medium access requirements. May indicate why WLAN power
         consumption is above an expected level. */
