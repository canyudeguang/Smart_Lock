 ...
 * Entries in this array is given by the Num_Responses member. A utility
 ...
 * @brief
 * Issues the
 * HCI_Set_Host_Controller_To_Host_Flow_Control Command to the Bluetooth
 * Device that is associated with the Bluetooth Protocol Stack specified
 * by the BluetoothStackID parameter.
 *
 * @details
 * Possible values for the Flow_Control_Enable parameter are:
 *
 *    QAPI_BLE_HCI_HOST_FLOW_CONTROL_ENABLE_OFF \n
 *    QAPI_BLE_HCI_HOST_FLOW_CONTROL_ENABLE_ON \n
 *    QAPI_BLE_HCI_HOST_FLOW_CONTROL_ENABLE_ACL_ON_SCO_OFF \n
 *    QAPI_BLE_HCI_HOST_FLOW_CONTROL_ENABLE_ACL_OFF_SCO_ON \n
 *    QAPI_BLE_HCI_HOST_FLOW_CONTROL_ENABLE_ACL_ON_SCO_ON
 *
 * This function blocks until either a result is returned from the
 * Bluetooth device or the function times out waiting for a response
 * from the Bluetooth device.
 ...
 *                                    call to qapi_BLE_BSC_Initialize().
 *
 * @param[in]   Flow_Control_Enable   Intended setting of this parameter.
 *                                    See above for possible values.
 *
 * @param[out]  StatusResult          Returned HCI status code.
 *
 ...
 *                  QAPI_BLE_BTPS_ERROR_HCI_RESPONSE_ERROR
 */
QAPI_BLE_DECLARATION int QAPI_BLE_BTPSAPI qapi_BLE_HCI_Set_Host_Controller_To_Host_Flow_Control(uint32_t BluetoothStackID, uint8_t Flow_Control_Enable, uint8_t *StatusResult);

/**
 * @brief
 * Issues the HCI_Host_Buffer_Size Command to the
 * Bluetooth device that is associated with the Bluetooth Protocol
 * Stack specified by the BluetoothStackID parameter.
 *
 * @details
 ...
 *                                                call to qapi_BLE_BSC_Initialize().
 *
 * @param[in]   Host_ACL_Data_Packet_Length       Maximum length of ACL data packets,
 *                                                up to 0xFFFF.
 *
 * @param[in]   Host_SCO_Data_Packet_Length       Maximum length of SCO data packets,
 *                                                up to 0xFF.
 *
 * @param[in]   Host_Total_Num_ACL_Data_Packets   Maximum number of ACL packets that
 *                                                can be held in the host, up to
 *                                                0xFFFF.
 *
 * @param[in]   Host_Total_Num_SCO_Data_Packets   Maximum number of SCO packets that
 *                                                can be held in the host, up to
 *                                                0xFFFF.
 *
 * @param[out]  StatusResult                      Returned HCI status code.
 ...
 *                  QAPI_BLE_BTPS_ERROR_HCI_RESPONSE_ERROR
 */
QAPI_BLE_DECLARATION int QAPI_BLE_BTPSAPI qapi_BLE_HCI_Host_Buffer_Size(uint32_t BluetoothStackID, uint16_t Host_ACL_Data_Packet_Length, uint8_t Host_SCO_Data_Packet_Length, uint16_t Host_Total_Num_ACL_Data_Packets, uint16_t Host_Total_Num_SCO_Data_Packets, uint8_t *StatusResult);

/**
 * @brief
 * Issues the HCI_Host_Number_Of_Completed_Packets
 * Command to the Bluetooth device that is associated with the
 * Bluetooth Protocol Stack specified by the BluetoothStackID parameter.
 *
 * @details
 * This command is used by the Host to indicate to the Host Controller
 * the number of HCI Data Packets that have been completed (processed)
 * for each connection since the last time this command was sent. This
 * tells the Host Controller that the corresponding buffer space has been
 * freed in the Host. This command should only be used when flow control
 * is on.
 *
 * This function blocks until either a result is returned from the
 * Bluetooth device or the function times out waiting for a response
 ...
 *                                                call to qapi_BLE_BSC_Initialize().
 *
 * @param[in]   Number_Of_Handles                 Number of packets in the provided
 *                                                array.  Must not be zero.
 *
 * @param[in]   HCI_Host_Completed_Packets_Info   Array of structures which pair up
 *                                                a connection handle and the number
 *                                                of packets which have been completed
 *                                                for that handle.
 *
 * @param[in]   WaitForResponse                   Boolean flag indicating whether this
 *                                                function call should wait until it
 *                                                gets a response from the Host
 *                                                Controller. Note that there is no response
 *                                                unless there is invalid data. Therefore,
 *                                                when the data is good, this function will
 *                                                stall until the timeout is reached.
 *                                                If the Host knows it is passing good
 *                                                data, it should set this flag
 *                                                to FALSE.
 *
 * @param[out]  StatusResult                      Returned HCI status code.
 *
 ...
 *                  QAPI_BLE_BTPS_ERROR_HCI_RESPONSE_ERROR
 */
QAPI_BLE_DECLARATION int QAPI_BLE_BTPSAPI qapi_BLE_HCI_Host_Number_Of_Completed_Packets(uint32_t BluetoothStackID, uint8_t Number_Of_Handles, qapi_BLE_HCI_Host_Completed_Packets_Info_t HCI_Host_Completed_Packets_Info[], uint8_t WaitForResponse, uint8_t *StatusResult);
...
/**
 * @brief
 * Issues the HCI_Read_Buffer_Size Command to the
 * Bluetooth device that is associated with the Bluetooth Protocol Stack
 * specified by the BluetoothStackID parameter.
 *
 * @details
 * This command reads the maximum size of the data portion of HCI ACL and
 * SCO Data Packets sent from the Host to the Host Controller (i.e.,
 * the Host Controller's size limits), and the total number of HCI
 * ACL and SCO Data Packets that can be stored in the data buffers of
 * the Host Controller. The Host must segment the data to be transmitted
 * according to these sizes, so that the HCI Data Packets will contain data
 * with up to these sizes. This command must be issued by the Host before
 * it sends any data to the Host Controller.
 *
 * This function blocks until either a result is returned from the
 * Bluetooth device or the function times out waiting for a response
 * from the Bluetooth device.
 ...
 * @param[out]  StatusResult                    Returned HCI status code.
 *
 * @param[out]  HC_ACL_Data_Packet_Length       Maximum length (in bytes) of the data
 *                                              portion of each HCI ACL Data Packet
 *                                              passed to the Host Controller.
 *
 * @param[out]  HC_SCO_Data_Packet_Length       Maximum length (in bytes) of the data
 *                                              portion of each HCI SCO Data Packet
 *                                              passed to the Host Controller.
 *
 * @param[out]  HC_Total_Num_ACL_Data_Packets   Maximum number of ACL Data Packets
 *                                              that can be stored in the Host
 *                                              Controller.
 *
 * @param[out]  HC_Total_Num_SCO_Data_Packets   Maximum number of SCO Data Packets
 *                                              that can be stored in the Host
 *                                              Controller.
 *
 * @return       Zero if successful.
 ...
 *                  QAPI_BLE_BTPS_ERROR_HCI_DRIVER_ERROR
 */
QAPI_BLE_DECLARATION int QAPI_BLE_BTPSAPI qapi_BLE_HCI_Read_Buffer_Size(uint32_t BluetoothStackID, uint8_t *StatusResult, uint16_t *HC_ACL_Data_Packet_Length, uint8_t *HC_SCO_Data_Packet_Length, uint16_t *HC_Total_Num_ACL_Data_Packets, uint16_t *HC_Total_Num_SCO_Data_Packets);
