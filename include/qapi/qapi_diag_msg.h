...
/*===========================================================================
                              Edit History
 $Header: //components/rel/core.ioe/1.0/api/services/qapi_diag_msg.h#9 $
 ...
 * directive. #pragma cannot be used directly in a pre-processor
 * definition, so if required, we need to rely on the _Pragma
 * operator supported by C99 and can be used inside a 
 * pre-processor definition. 
 *  BUT,
 * Since IAR does 4-byte alignment by default, these directives 
 * will be defined to nothing for IAR. Keeping the directives in 
 * place in the macro definitions as a reminder that alignment 
 * matters for these macros.
 *  
 * This header file supports GCC, ARMCT and IAR. If customer 
...
{                     
   const char *format;
   uint32_t diag_Msg_ID;
}qapi_Diag_Msg_Compact_t;  /*! This will take up 4 bytes in client's execution region. Will initially have the format string address, but can be replaced by MessageID if compaction is enabled for QAPI */


/**
 * See QAPI_DIAG_MSG_ARG3().
 */
 ...
 *                      Module IDs through 225 are reserved for QCOM modules. OEM modules can use 226 through 255.
 *                      Module IDs and their corresponding tech areas can be maintained as configuration information
