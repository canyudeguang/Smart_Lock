...
/** @file qapi_zb_cl_ias_ace.h
   ...
   APIs are provided to create an IAS ACE server and client cluster.  Cluster
   creation also registers an event callback that is used to receive events from
   the IAS ACE cluster.  Client events are for command responses and server
...
{
   /** Flag indicating if all zone statuses have been queried. */
...
typedef struct qapi_ZB_CL_IASACE_Get_Zone_Status_s
{
   ...
   /** Flag indicating if the server should include only the zones whose status
       is equal to one or more values in the ZoneStatusMask. */
...
/**
   Structure representing the data for events received by a IAS ACE client
   cluster.
*/
...
typedef struct qapi_ZB_CL_IASACE_Server_Event_Data_s
{
   ...
   union
   {
      ...
      /** Information for Arm command. */
      qapi_ZB_CL_IASACE_Arm_t        Arm;

      /** Information for Bypass command. */
...
/**
   @brief This function can be used to populate an attribute list with the
          attributes required for an IAS ACE cluster.
...
/**
   @brief Set the Arm/Disarm code on the IAS ACE server.
...
/**
   @brief Set the Panel Status on the IAS ACE server.

   @param[in] Cluster Handle for the IAS ACE server cluster.
   @param[in] Request Information for the panel status.

   @return
     - QAPI_OK if the command executed successfully.
     - A negative value if there was an error.
*/
qapi_Status_t qapi_ZB_CL_IASACE_Server_Set_Panel_Status(qapi_ZB_Cluster_t Cluster, const qapi_ZB_CL_IASACE_Panel_Status_Info_t *Request);

/**
   @brief Add a zone table entry on the IAS ACE server.
...
/**
   @brief Delete a zone table entry on the IAS ACE server.
...
/**
   @brief Set the zone status of a zone in the zone table on the IAS ACE
          server.
...
/**
   ...
   @param[in] Permit  Flag indicating if bypassing the zone is permitted.
...
/**
   ...
   @param[in]  Cluster      Handle for the IAS ACE server cluster.
   @param[in]  ZoneID       ID of the zone to configure.
   @param[in]  Bypass       Flag indicating if the zone should be bypassed.
...
/**
   @brief Retrieve a zone table entry by ID from the IAS ACE
          server.

   @param[in]  Cluster  Handle for the IAS ACE server cluster.
   @param[in]  ZoneID   ID of the zone to be found.
   @param[out] ZoneInfo If successful, retrieved zone information.
                        The ZoneLabel element of the table entry will
                        point to a character array with a length of QAPI_ZB_CL_IASACE_ZONE_LABEL_MAX_LENGTH + 1.
   ...
   @return
     ...
     - A negative value if there was an error.
*/
qapi_Status_t qapi_ZB_CL_IASACE_Server_Find_Zone_By_ID(qapi_ZB_Cluster_t Cluster, uint8_t ZoneID, qapi_ZB_CL_IASACE_Zone_Table_Entry_t *ZoneInfo);

/**
   @brief Retrieve a zone table entry by address from the IAS ACE
          server.
   ...
   @return
     ...
     - A negative value if there was an error.
*/
qapi_Status_t qapi_ZB_CL_IASACE_Server_Find_Zone_By_Address(qapi_ZB_Cluster_t Cluster, uint64_t ZoneAddress, qapi_ZB_CL_IASACE_Zone_Table_Entry_t *ZoneInfo);

/**
   @brief Retrieve a list of zones currently registerd with the IAS ACE
          server.
