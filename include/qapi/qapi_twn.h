...
/** Represents a handle for a Thread Wireless Network. It is
...
/**
   Union that represents a single IPv6 address (16 bytes). Note that this address
   is expected to be in network byte order (i.e. big-endian), regardless of the
   mode being used to access it (u8/u16/u32).
*/
...
{
   /** Device's extended address. If non-zero, overwrites the generated Thread
       extended address. */
   ...
   /** Indicates whether the device uses MAC level security on all data requests. */
...
typedef struct qapi_TWN_Network_Configuration_s
{
   ...
   /** Thread Network name. NULL terminator only required if less than
       QAPI_OPEN_THREAD_NETWORK_NAME_SIZE bytes in length. */
...
/**
   ...
   with the qapi_TWN_Add_Border_Router() and qapi_TWN_Remove_Border_Router() APIs.
*/
typedef struct qapi_TWN_Border_Router_s
{
   ...
   /** Configuration flags for the border router (see QAPI_TWN_BORDER_ROUTER_FLAG_*). */
   uint32_t                      Flags;
...
typedef struct qapi_TWN_Joiner_Info_s
{
   ...
   /** String representing the software version. Pointer must remain valid
       until joining has completed (i.e. Joiner_Stop() or callback has
       occurred). */
...
#define QAPI_TWN_SEC_POLICY_FLAGS_ALLOW_OUT_OF_BAND_MASTER_KEY          (1 << 0) /**< Determines if the Master Key is allowed to be
                                                                                      transmitted out-of-band i.e. to an external
                                                                                      commissioner via MGMT_GET.rsp messages. */
#define QAPI_TWN_SEC_POLICY_FLAGS_ALLOW_NATIVE_COMMISSIONING            (1 << 1) /**< Allows commissioning over 802.15.4 to an
                                                                                      external commissioner that only knows the PSKc. */
#define QAPI_TWN_SEC_POLICY_FLAGS_ALLOW_THREAD_1X_ROUTERS               (1 << 2) /**< Allows Thread 1.x devices to act as routers. */
#define QAPI_TWN_SEC_POLICY_FLAGS_ALLOW_EXTERNAL_COMMISSIONER_AUTH      (1 << 3) /**< Allows commissioning sessions based on the PSKc
                                                                                      to be established, and also allows changes to
                                                                                      the Commissioner Dataset to be made by on-mesh
                                                                                      devices. */
#define QAPI_TWN_SEC_POLICY_FLAGS_ALLOW_THREAD_1X_BEACONS               (1 << 4) /**< Allows Thread 1.x beacons to be sent in response
                                                                                      to a beacon scan. If disabled, the only way to
                                                                                      discover a network is via MLE Discover Requests. */
...
/**
   ...
   @param[in]  CB_Param     User-specified parameter that will be provided
                            to the TWN event callback function.
...
/**
   ...
   @param[in] TWN_Handle Handle of the TWN instance as provided by a
                         successful call to qapi_TWN_Initialize().
   @param[in] Prefix     Prefix associated with the border router to be
                         removed.
