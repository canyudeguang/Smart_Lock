...
/* The size of the remote device list for the demo application. */
#define DEVICE_LIST_SIZE                           (4)
...
#define SEND_DATA_DISPLAY_FREQUENCY                (0)
...
#define SEND_INFO_LIST_ENTRY_FLAG_RESTART_TIMER                         (0x02)
#define SEND_INFO_LIST_ENTRY_FLAG_TRANSITION_TO_SOM                     (0x04)
...
#define HMI_DEMO_TRANSITION_DATA_SIZE(__device_count)                   (sizeof(HMI_Demo_Transition_Data_t) + ((__device_count) * sizeof(Device_List_Entry_t)))

#define HMI_DEMO_TRANSITION_DATA_CONFIG_IS_SLEEPY_SHIFT                 (0x0000)
#define HMI_DEMO_TRANSITION_DATA_CONFIG_SRC_ADDR_MODE_SHIFT             (0x0001)
#define HMI_DEMO_TRANSITION_DATA_CONFIG_DEST_ADDR_MODE_SHIFT            (0x0003)
#define HMI_DEMO_TRANSITION_DATA_CONFIG_SECURITY_LEVEL_SHIFT            (0x0005)
#define HMI_DEMO_TRANSITION_DATA_CONFIG_KEY_ID_MODE_SHIFT               (0x0008)

#define HMI_DEMO_TRANSITION_DATA_CONFIG_IS_SLEEPY_MASK                  (0x0001)
#define HMI_DEMO_TRANSITION_DATA_CONFIG_SRC_ADDR_MODE_MASK              (0x0003)
#define HMI_DEMO_TRANSITION_DATA_CONFIG_DEST_ADDR_MODE_MASK             (0x0003)
#define HMI_DEMO_TRANSITION_DATA_CONFIG_SECURITY_LEVEL_MASK             (0x0007)
#define HMI_DEMO_TRANSITION_DATA_CONFIG_KEY_ID_MODE_MASK                (0x0003)
...
const char *HMI_Status_To_String(uint8_t Status);
...
static QCLI_Command_Status_t HMI_Cmd_Initialize(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   if(HMI_Demo_Context.Interface_ID == 0)
   {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[1]), 0, 1)))
      {
         ...
         {
            /* Set the extended address for the local interface. */
            ExtendedAddress = DEFAULT_EUI64_PREFIX + Parameter_List[0].Integer_Value;
            Result = qapi_HMI_MLME_Set_Request(HMI_Demo_Context.Interface_ID, QAPI_HMI_PIB_MAC_EXTENDED_ADDRESS, 0, sizeof(ExtendedAddress), &ExtendedAddress, &Status);
         ...
         if(Hex_String_To_ULL(Parameter_List[0].String_Value, &Ext_Address))
         {
            ...
            if(Ret_Val == QCLI_STATUS_SUCCESS_E)
            {
               ...
               if(Device_Index < DEVICE_LIST_SIZE)
               {
                  ...
                  HMI_Demo_Context.Device_List[Device_Index].DeviceIndex = Device_Index;
         ...
         case QAPI_HMI_EVENT_TYPE_MCPS_DATA_CONFIRM_E:
            ...
            if(Send_Info_List_Entry != NULL)
            {
                  ...
                  (HMI_Event->Event_Data.MCPS_Data_Confirm.Status == QAPI_HMI_STATUS_CODE_CHANNEL_ACCESS_FAILURE))
               {
                  ...
                  {
                     if(Send_Info_List_Entry->Flags & SEND_INFO_LIST_ENTRY_FLAG_TRANSITION_TO_SOM)
                     {
                        /* Try to go back to SOM. */
                        PAL_Switch_Operating_Mode(OPERATING_MODE_SOM_E);
                     }
                  }
               }
               else
...
static void Periodic_Send_Timer_CB(uint32_t data)
{
   ...
   if(Send_Info_List_Entry != NULL)
   {
      ...
      if(!(Send_Info_List_Entry->Flags & SEND_INFO_LIST_ENTRY_FLAG_WAITING_CONFIRM))
      {
         ...
         if(Result == QAPI_OK)
         {
            if(Send_Info_List_Entry->Flags & SEND_INFO_LIST_ENTRY_FLAG_RESTART_TIMER)
            {
               Send_Info_List_Entry->Flags &= ~SEND_INFO_LIST_ENTRY_FLAG_RESTART_TIMER;

               /* Restart the timer. */
               Set_Timer_Attr.time                   = Send_Info_List_Entry->Period;
               Set_Timer_Attr.reload                 = true;
               Set_Timer_Attr.max_deferrable_timeout = Send_Info_List_Entry->Period;
               Set_Timer_Attr.unit                   = QAPI_TIMER_UNIT_MSEC;
               Result = qapi_Timer_Set(Send_Info_List_Entry->Timer, &Set_Timer_Attr);
            }
         }
...
void Initialize_HMI_Demo(qbool_t IsColdBoot)
{
   ...
   if(Result == QAPI_OK)
   {
      ...
      {
         if((Data_Size >= sizeof(HMI_Demo_Transition_Data_t)) && (Data_Size >= HMI_DEMO_TRANSITION_DATA_SIZE(Transition_Data->Device_Count)))
         {
            /* Restore backed up data. */
            HMI_Demo_Context.Interface_ID = Transition_Data->Interface_ID;
            HMI_Demo_Context.PAN_ID       = Transition_Data->PAN_ID;
            HMI_Demo_Context.Is_Sleepy    = (qbool_t)(((Transition_Data->Config >> HMI_DEMO_TRANSITION_DATA_CONFIG_IS_SLEEPY_SHIFT) & HMI_DEMO_TRANSITION_DATA_CONFIG_IS_SLEEPY_MASK) != 0);
            HMI_Demo_Context.SrcAddrMode  = ((Transition_Data->Config >> HMI_DEMO_TRANSITION_DATA_CONFIG_SRC_ADDR_MODE_SHIFT) & HMI_DEMO_TRANSITION_DATA_CONFIG_SRC_ADDR_MODE_MASK);
            HMI_Demo_Context.DstAddrMode  = ((Transition_Data->Config >> HMI_DEMO_TRANSITION_DATA_CONFIG_DEST_ADDR_MODE_MASK) & HMI_DEMO_TRANSITION_DATA_CONFIG_DEST_ADDR_MODE_MASK);
            HMI_Demo_Context.HMI_Security.SecurityLevel = ((Transition_Data->Config >> HMI_DEMO_TRANSITION_DATA_CONFIG_SECURITY_LEVEL_SHIFT) & HMI_DEMO_TRANSITION_DATA_CONFIG_SECURITY_LEVEL_MASK);

            if(HMI_Demo_Context.HMI_Security.SecurityLevel != 0)
            {
               HMI_Demo_Context.HMI_Security.KeyIdMode = ((Transition_Data->Config >> HMI_DEMO_TRANSITION_DATA_CONFIG_KEY_ID_MODE_SHIFT) & HMI_DEMO_TRANSITION_DATA_CONFIG_KEY_ID_MODE_SHIFT);
               HMI_Demo_Context.HMI_Security.KeyIndex  = Transition_Data->KeyIndex;
               HMI_Demo_Context.HMI_Security.KeySource = KeyDescriptorList[HMI_Demo_Context.HMI_Security.KeyIndex].Source;
            }

            /* Restore the device list. */
            for(Index = 0; Index < Transition_Data->Device_Count; Index++)
            {
               memscpy(&(HMI_Demo_Context.Device_List[Transition_Data->Device_List[Index].DeviceIndex]), sizeof(Device_List_Entry_t),&(Transition_Data->Device_List[Index]), sizeof(Device_List_Entry_t));
            }

            /* Restore the send info if it exists. */
            if(Transition_Data->Send_Info.Expire_Time != 0)
            {
               HMI_Demo_Context.Send_Info_List = CreateSendInfoListEntry(Transition_Data->Send_Info.Device_Index, Transition_Data->Send_Info.Period);
               if(HMI_Demo_Context.Send_Info_List != NULL)
               {
                  HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.SrcAddrMode = HMI_Demo_Context.SrcAddrMode;
                  HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.DstAddrMode = HMI_Demo_Context.DstAddrMode;
                  HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.DstPANId    = HMI_Demo_Context.PAN_ID;
                  HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.MSDULength  = Transition_Data->Send_Info.MSDULength;
                  HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.MSDU        = DataRequest_MSDU;
                  HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.MSDUHandle  = Get_Next_MSDUHandle();
                  HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.TxOptions   = Transition_Data->Send_Info.TxOptions;
                  HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.Security    = &(HMI_Demo_Context.HMI_Security);
                  if(HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.SrcAddrMode == QAPI_HMI_ADDRESS_MODE_SHORT_ADDRESS)
                  {
                     HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.DstAddr.ShortAddress = HMI_Demo_Context.Device_List[HMI_Demo_Context.Send_Info_List->Device_Index].ShortAddr;
                  }
                  else if(HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.SrcAddrMode == QAPI_HMI_ADDRESS_MODE_EXTENDED_ADDRESS)
                  {
                     HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.DstAddr.ExtendedAddress = HMI_Demo_Context.Device_List[HMI_Demo_Context.Send_Info_List->Device_Index].ExtAddr;
                  }

                  /* Flag that the demo should restart the timr for the next
                     transmission and re-enter SOM mode when the packet has been
                     sent. */
                  HMI_Demo_Context.Send_Info_List->Flags |= (SEND_INFO_LIST_ENTRY_FLAG_RESTART_TIMER | SEND_INFO_LIST_ENTRY_FLAG_TRANSITION_TO_SOM);

                  HMI_Demo_Context.NextExpireTime = Transition_Data->Send_Info.Expire_Time;
               }
            }
         }
...
void Start_HMI_Demo(qbool_t IsColdBoot)
{
   ...
   {
      /* Start the timer. */
      Result = qapi_Timer_set_absolute(HMI_Demo_Context.Send_Info_List->Timer, HMI_Demo_Context.NextExpireTime, false);

      if(Result != QAPI_OK)
      {
         Display_Function_Error(HMI_Demo_Context.QCLI_Handle, "qapi_Timer_set_absolute", Result);

         DeleteSendInfoListEntry(HMI_Demo_Context.Send_Info_List->Device_Index);
      }
   }
}
...
qbool_t HMI_Prepare_Mode_Switch(Operating_Mode_t Next_Mode)
{
   ...
   uint8_t                     Send_Info_Count;
   Send_Info_List_Entry_t     *Send_Info_List_Entry;
   HMI_Demo_Transition_Data_t *Transition_Data;

   /* Make sure the demo and HMI layer have been initialized successfully. */
   if((HMI_Demo_Context.QCLI_Handle != NULL) && (HMI_Demo_Context.Interface_ID != 0))
   {
#if 0 //xxx not applicable until prepare function is truly supported.
      /* Make sure we are in a state where we can transition out of FOM.  This
         means there are no outstanding receives and at most one oustanding
         send. */
      if((HMI_Demo_Context.Receive_Info_List == NULL) &&  ((HMI_Demo_Context.Send_Info_List == NULL) || ((HMI_Demo_Context.Send_Info_List->Next_Entry == NULL) && (HMI_Demo_Context.Send_Info_List_Entry->Period != 0)))
#else
      if(true)
#endif
      {
         /* Determine how many devices will need to be backed up. */
         Device_Count = 0;
         for(Index = 0; Index < DEVICE_LIST_SIZE; Index ++)
         {
            if(HMI_Demo_Context.Device_List[Index].Flags & DEVICE_LIST_ENTRY_FLAG_ENTRY_IN_USE)
            {
               Device_Count ++;
            }
         }

         /* Determine the number of send info entries. */
         Send_Info_Count        = 0;
         Send_Info_List_Entry = HMI_Demo_Context.Send_Info_List;
         while(Send_Info_List_Entry != NULL)
         {
            Send_Info_Count ++;
            Send_Info_List_Entry = Send_Info_List_Entry->Next_Entry;
         }

         /* Allocate the transition memory. */
         Result = qapi_OMSM_Alloc(QAPI_OMSM_DEFAULT_AON_POOL, OMSM_CLIENT_ID_HMI_DEMO_E, HMI_DEMO_TRANSITION_DATA_SIZE(Device_Count), (void **)&Transition_Data);

         if((Result == QAPI_OK) && (Transition_Data != NULL))
         {
            memset(Transition_Data, 0, HMI_DEMO_TRANSITION_DATA_SIZE(Device_Count));
            Transition_Data->Device_Count = Device_Count;
            ...
            {
               qapi_OMSM_Free(QAPI_OMSM_DEFAULT_AON_POOL, OMSM_CLIENT_ID_HMI_DEMO_E);
               Display_Function_Error(HMI_Demo_Context.QCLI_Handle, "qapi_OMSM_Commit", Result);
               Ret_Val = false;
            }
         }
         else
         {
            Display_Function_Error(HMI_Demo_Context.QCLI_Handle, "qapi_OMSM_Alloc", Result);
            Ret_Val = false;
         }
      }
      else
      {
         QCLI_Printf(HMI_Demo_Context.QCLI_Handle, "Transition not allowed\n");
         Ret_Val = false;
      }
   }
   else
   ...
   if((HMI_Demo_Context.QCLI_Handle != NULL) && (HMI_Demo_Context.Interface_ID != 0))
   {
      ...
      if((Result == QAPI_OK) && (Transition_Data != NULL))
      {
         ...
         if((Data_Size >= sizeof(HMI_Demo_Transition_Data_t)) && (Data_Size >= HMI_DEMO_TRANSITION_DATA_SIZE(Transition_Data->Device_Count)))
         {
            ...
            Transition_Data->Interface_ID  = HMI_Demo_Context.Interface_ID;
            Transition_Data->PAN_ID        = HMI_Demo_Context.PAN_ID;
            Transition_Data->KeyIndex      = HMI_Demo_Context.HMI_Security.KeyIndex;
            Transition_Data->Config        = (HMI_Demo_Context.Is_Sleepy ? (HMI_DEMO_TRANSITION_DATA_CONFIG_IS_SLEEPY_MASK < HMI_DEMO_TRANSITION_DATA_CONFIG_IS_SLEEPY_SHIFT) : 0);
            Transition_Data->Config       |= (HMI_Demo_Context.SrcAddrMode & HMI_DEMO_TRANSITION_DATA_CONFIG_SRC_ADDR_MODE_MASK) << HMI_DEMO_TRANSITION_DATA_CONFIG_SRC_ADDR_MODE_SHIFT;
            Transition_Data->Config       |= (HMI_Demo_Context.DstAddrMode & HMI_DEMO_TRANSITION_DATA_CONFIG_DEST_ADDR_MODE_MASK) << HMI_DEMO_TRANSITION_DATA_CONFIG_DEST_ADDR_MODE_SHIFT;
            Transition_Data->Config       |= (HMI_Demo_Context.HMI_Security.SecurityLevel & HMI_DEMO_TRANSITION_DATA_CONFIG_SECURITY_LEVEL_MASK) << HMI_DEMO_TRANSITION_DATA_CONFIG_SECURITY_LEVEL_SHIFT;
            Transition_Data->Config       |= (HMI_Demo_Context.HMI_Security.KeyIdMode & HMI_DEMO_TRANSITION_DATA_CONFIG_KEY_ID_MODE_MASK) << HMI_DEMO_TRANSITION_DATA_CONFIG_KEY_ID_MODE_SHIFT;
            if(HMI_Demo_Context.Send_Info_List != NULL)
            {
               Result = qapi_Timer_Get_Time_Info(HMI_Demo_Context.Send_Info_List->Timer, QAPI_TIMER_TIMER_INFO_ABS_EXPIRY, &(Transition_Data->Send_Info.Expire_Time));

               if(Result != QAPI_OK)
               {
                  /* Assume that the timer has already expired. */
                  Transition_Data->Send_Info.Expire_Time = 1;
               }

               Transition_Data->Send_Info.Device_Index = HMI_Demo_Context.Send_Info_List->Device_Index;
               Transition_Data->Send_Info.Flags        = HMI_Demo_Context.Send_Info_List->Flags;
               Transition_Data->Send_Info.MSDULength   = HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.MSDULength;
               Transition_Data->Send_Info.TxOptions    = HMI_Demo_Context.Send_Info_List->MCPS_Data_Request.TxOptions;
               Transition_Data->Send_Info.Period       = HMI_Demo_Context.Send_Info_List->Period;
            }

            /* Backup the device table information. */
            for(Index = 0, Device_Count = 0; Index < DEVICE_LIST_SIZE && (Device_Count < Transition_Data->Device_Count); Index ++)
            {
               if(HMI_Demo_Context.Device_List[Index].Flags & DEVICE_LIST_ENTRY_FLAG_ENTRY_IN_USE)
               {
                  memscpy(&(Transition_Data->Device_List[Device_Count]), sizeof(Device_List_Entry_t), &(HMI_Demo_Context.Device_List[Index]), sizeof(Device_List_Entry_t));
                  Device_Count ++;
               }
            }
