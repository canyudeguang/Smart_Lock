...
REM Validate the board variant. Default: carrier
IF /I "%~3" == "" (
...
REM Get the technology supported.
IF /I "%~5" == "" (
   SET TECHNOLOGY=ALL
) ELSE IF /I "%5" == "all" (
   SET TECHNOLOGY=ALL
   ...
   SET TECHNOLOGY=I15P4
) ELSE (
   ECHO Invalid Technology: %5%
   GOTO:Usage
)
...
IF /I "%~6" == "" (
   ...
   IF EXIST "%~6" (
      ...
      ECHO Invalid NVM file path: %6%
      GOTO:Usage
   )
)

@ECHO ****************************************************************************
@ECHO                      Building Quartz Passthrough Application for %CHIPSET_VERSION% Chipset
@ECHO                      RTOS             %RTOS%
...
SET CSrcs=sbrk.c
SET CSrcs=%CSrcs% app_main.c
SET CSrcs=%CSrcs% PtUart.c
SET CSrcs=%CSrcs% QPKTZR.c
...
SET Includes=-I"%RootDir%\quartz\demo\Passthrough_demo\src"
SET Includes=%Includes% -I"%RootDir%\build\ms"
SET Includes=%Includes% -I"%RootDir%\include"
SET Includes=%Includes% -I"%RootDir%\include\qapi"
SET Includes=%Includes% -I"%RootDir%\include\bsp"
SET Includes=%Includes% -I"%RootDir%\quartz\bluetopia\overlay\include"
SET Includes=%Includes% -I"%RootDir%\quartz\bluetopia\overlay\ss1btle"
SET Includes=%Includes% -I"%RootDir%\quartz\bluetopia\overlay\btpskrnl"
SET Includes=%Includes% -I"%RootDir%\quartz\bluetopia\overlay\hcitrans"
SET Includes=%Includes% -I"%RootDir%\quartz\bluetopia\overlay\btpscfg"
SET Includes=%Includes% -I"%RootDir%\quartz\bluetopia\overlay\btpsver"
SET Includes=%Includes% -I"%RootDir%\quartz\bluetopia\overlay\hcidrv"
SET Includes=%Includes% -I"%RootDir%\bluetopia\Build\Core\include"
SET Includes=%Includes% -I"%RootDir%\bluetopia\Build\Core\ss1btle"
SET Includes=%Includes% -I"%RootDir%\bluetopia\Build\Core\btpskrnl"
SET Includes=%Includes% -I"%RootDir%\bluetopia\Build\Core\hcitrans"
SET Includes=%Includes% -I"%RootDir%\bluetopia\Build\Core\btpscfg"
SET Includes=%Includes% -I"%RootDir%\bluetopia\Build\Core\btpsver"
SET Includes=%Includes% -I"%RootDir%\bluetopia\Build\Core\hcidrv"
SET Includes=%Includes% -I"%RootDir%\quartz\lpwnet\hmi\inc"
SET Includes=%Includes% -I"%RootDir%\core\%CHIPSET_VERSION%\rom\release\api\power\unpa"
SET Includes=%Includes% -I"%RootDir%\core\%CHIPSET_VERSION%\rom\release\api\power\uSleep"
SET Includes=%Includes% -I"%RootDir%\core\%CHIPSET_VERSION%\rom\release\api\kernel\libstd\stringl"
SET Includes=%Includes% -I"%RootDir%\core\%CHIPSET_VERSION%\rom\release\api\buses\uart"
SET Includes=%Includes% -I"%RootDir%\wlan_host\qapi\include"
SET Includes=%Includes% -I"%RootDir%\wlan_host\host_driver\custom\include"
SET Includes=%Includes% -I"%RootDir%\wlanfw_cmn.ioe\include"
...
IF /I "%TECHNOLOGY%" == "BLE" (
   SET COpts=%COpts% -DDISABLE_I15P4
) ELSE IF /I "%TECHNOLOGY%" == "I15P4" (
   SET COpts=%COpts% -DDISABLE_BLE
)
...
IF /I "%BOARD_VARIANT%" == "CDB" (
    ...
    python %SectoolsQdnDir%\sectools.py iot -p 4020 -g m0 -i %RootDir%\bin\cortex-m0\threadx\ioe_ram_m0_threadx_ipt.mbn -k %SectoolsCertsDir%\qpsa_rootca.key -c %SectoolsCertsDir%\qpsa_rootca.cer --cfg_oem_id=0x0000 --cfg_model_id=0x0000 -o . -s
) ELSE (
...
:Usage
ECHO.
ECHO Usage: build.bat [RTOS (FreeRTOS = f, ThreadX = t)] [chipset variant (4020, 4024, 4025)] [board variant (Carrier = c, DUT = d, CDB = cdb)] [chipset revision (1.2 = 1p2, 2.0 = 2p0)] [enabled techs (all, ble, i15p4)] [NVM file]
GOTO:EndOfFile
