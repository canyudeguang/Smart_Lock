...
/* Function prototypes. */
//static QCLI_Command_Status_t ZCL_LevelControl_MoveToLevel(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List);
...
const QCLI_Command_t ZigBee_LevelControl_CMD_List[] =
{
   ...
   {ZCL_LevelControl_MoveToLevel,      false,  "MoveToLevel",  "[DevId][ClientEndpoint][WithOnOff][Level0-255][Time]",            "Sends a Move To Level command to a Level Control server."},
...
uint32_t ZCL_LevelControl_MoveToLevel(uint32_t DevId, uint32_t  DimmerEndPoint, qbool_t OnOff, uint8_t level, uint16_t time)
{
   ...
   if((GetZigBeeHandle() != NULL))
   {
        ...
        DeviceId    = DevId;
        ClusterInfo = ZCL_FindClusterByEndpoint(DimmerEndPoint, QAPI_ZB_CL_CLUSTER_ID_LEVEL_CONTROL, ZCL_DEMO_CLUSTERTYPE_CLIENT);
        WithOnOff   = OnOff;
        Level       = level;
        TransTime   = time;
   ...
   if((ZB_Handle != NULL) && (Cluster != NULL) && (EventData != NULL))
   {
      ...
      switch(EventData->Event_Type)
      {
         ...
         case QAPI_ZB_CL_LEVELCONTROL_SERVER_EVENT_TYPE_TRANSITION_E:
            LOG_INFO("\n\n\nLevelControl Server Transistion Event:\n");
            if((EventData->Data.Transition.Level != QAPI_ZB_CL_LEVELCONTROL_INVALID_LEVEL) && (EventData->Data.Transition.TransitionTime != QAPI_ZB_CL_LEVELCONTROL_INVALID_TRANSITION_TIME))
            {
               ...
               LOG_INFO("  TransitionTime: %d\n", EventData->Data.Transition.TransitionTime);
               LOG_INFO("  WithOnOff:      %s\n", EventData->Data.Transition.WithOnOff ? "True" : "False");
               LOG_INFO("  \n\n\n\n" );
               level_val = (EventData->Data.Transition.Level/5);

               LOG_INFO(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  \n\n\n");
               LOG_INFO("Level Val: %d\n", level_val);
               BLUE_LED_CONFIG(50, level_val);
               Update_dimmer_value(EventData->Data.Transition.Level);
#if 0
               if (EventData->Data.Transition.Level == 0)
                   BLUE_LED_CONFIG (50, 0);
               else if (EventData->Data.Transition.Level >= 1 && EventData->Data.Transition.Level <= 100)
               {
                   LOG_INFO("  \n Else if Calling Blue led\n\n");
                   BLUE_LED_CONFIG (50, 1);
               }
               else
               {
                   LOG_INFO("  \n Else Calling Blue led\n\n");
                   BLUE_LED_CONFIG (50, (EventData->Data.Transition.Level - 100 )/10);
               }
#endif
            ...
            {
               LOG_INFO("  Stop level transition.\n");
            }
            break;
   ...
   if((ZB_Handle != NULL) && (Cluster != NULL) && (EventData != NULL))
   {
      switch(EventData->Event_Type)
      {
         case QAPI_ZB_CL_LEVELCONTROL_CLIENT_EVENT_TYPE_DEFAULT_RESPONSE_E:
            LOG_INFO("LevelControl Client Default Response:\n");
        ...
        break;
         ...
         default:
            LOG_INFO("Unhandled LevelControl client event %d.\n", EventData->Event_Type);
            break;
      }
