   ...
   if((ZB_Handle != NULL) && (Cluster != NULL) && (EventData != NULL))
   {
      switch(EventData->Event_Type)
      {
         ...
         case QAPI_ZB_CL_SCENES_CLIENT_EVENT_TYPE_VIEW_SCENE_RESPONSE_E:
            ...
            {
               QCLI_Printf(ZCL_Scenes_QCLI_Handle, "    ExtFildSet:    %d", EventData->Data.View_Scene_Response.ExtensionFieldSets[0].ClusterId);

               for(Index = 1; Index < EventData->Data.View_Scene_Response.ExtensionFieldCount; Index ++)
               {
                  QCLI_Printf(ZCL_Scenes_QCLI_Handle, ", %d", EventData->Data.View_Scene_Response.ExtensionFieldSets[Index].ClusterId);
               }
