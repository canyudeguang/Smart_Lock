 ...
 ******************************************************************************/
/*
  * This file contains functions and definitions for Server node.
 */
/*****************************************************************************/
...
/* Default TTL value used to send model messages. Referred by server models as well */
...
static QMESH_DEVICE_IDENTIFIER_DATA_T device_data =
{
    {0x691B, 0xDC08, 0x1021, 0x0B0E,
     0x0A0C, 0x000B, 0x0E0A, 0x0007},   /* uuid */
    0,                                  /* uri_length */
    NULL,                               /* uri */
    0,                                  /* oob_info_mask  */
    {                                   /* dev_caps */
 ...
 */
/*! \brief Handles QMESH_PROV_COMPLETE_DATA event received from the mesh stack  
 *             when provisioning is complete. The provisioned device details are stored the peer device list.
...
static void HandleProvisionCompleteEvent (QMESH_APP_EVENT_DATA_T event_data)
{
    ...
    if(QMESH_RESULT_SUCCESS == event_data.status)
    {
        ...
        /* Provisioning is successful. Remote side should send "Config AppKey Add message" to 
             ...
             (Generic OnOff Server model) to the Application key. 
             QMESH_CONFIG_APP_BIND_UNBIND_EVENT event is 
...
static void QmeshInitHealthServerData(const QMESH_DEVICE_COMPOSITION_DATA_T *dev_comp)
{
    ...
    g_health_server_state.pub_info = &server_publish_state [1];
...
static void HandleAppKeyBindEvent(QMESH_APP_EVENT_DATA_T event_data)
{
    QMESH_CONFIG_APP_BIND_EVENT_T *bind_event = NULL;
    ...
    QCLI_LOGI (mesh_group, "\r\nElement Address: %d", 
                            bind_event->ele_model_info.p_ele->element_data->unicast_addr);
    ...
    QCLI_LOGE (mesh_group, "\r\nNode ready to process Generic OnOff messages \n");
    QCLI_LOGE (mesh_group, "\r\nSend Generic OnOff message from Client node...\n");
    /*
        Now this device can process Generic OnOff server messages.

        Note: Generic OnOff client node should have been provisioned to send the OnOff client
        messages
    */
}
...
static void ProcessMeshEvent (QMESH_APP_EVENT_DATA_T event_data_cb)
{
    ...
    switch(operation_event)
    {
        ...
        case QMESH_PROVISION_STATUS_EVENT:
        {
            ...
            QCLI_LOGE (mesh_group, "\r\nProvisioning : %s (%d)",
                       prov_status[*(QMESH_PROV_STATUS_T *) event_data_cb.param >= 
                       QMESH_PROV_STATUS_MAX ?
                       QMESH_PROV_STATUS_MAX : 
                       *(QMESH_PROV_STATUS_T *) event_data_cb.param],
                       *(QMESH_PROV_STATUS_T *) event_data_cb.param);
        }
        break;
...
QmeshResult InitializeMesh(QMESHAPP_CONTEXT_T *app_context)
{
    ...
    /* Initialize Generic OnOff server model */
    QmeshGenericOnOffServerAppInit(&g_onoff_model_context);
    QmeshGenericOnOffPublicationHandler(&server_model_data[2]);
