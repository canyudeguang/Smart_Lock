...
{
   /* ClusterID                                    ClusterName     InitFunc                          ServerCreateFunc                      Func */
   ...
   { QAPI_ZB_CL_CLUSTER_ID_GROUPS,                  "GROUP",       Initialize_ZCL_Groups_Demo,       ZCL_Groups_Demo_Create_Server,       ZCL_Groups_Demo_Create_Client   },
   { QAPI_ZB_CL_CLUSTER_ID_SCENES,                  "SCENE",       Initialize_ZCL_Scenes_Demo,       ZCL_Scenes_Demo_Create_Server,       ZCL_Scenes_Demo_Create_Client   },
   { QAPI_ZB_CL_CLUSTER_ID_ONOFF,                   "ONOFF",        Initialize_ZCL_OnOff_Demo,        ZCL_OnOff_Demo_Create_Server,        ZCL_OnOff_Demo_Create_Client   },
   { QAPI_ZB_CL_CLUSTER_ID_LEVEL_CONTROL,           "LVL", Initialize_ZCL_LevelControl_Demo, ZCL_LevelControl_Demo_Create_Server, ZCL_LevelControl_Demo_Create_Client   },
   { QAPI_ZB_CL_CLUSTER_ID_ALARMS,                  "ALARM",       Initialize_ZCL_Alarms_Demo,       ZCL_Alarms_Demo_Create_Server,       ZCL_Alarms_Demo_Create_Client   },
   { QAPI_ZB_CL_CLUSTER_ID_TIME,                    "Time",         Initialize_ZCL_Time_Demo,         NULL,                                ZCL_Time_Demo_Create_Client   },
#if 1
   {QAPI_ZB_CL_CLUSTER_ID_OTA_UPGRADE,             "OTA",          NULL/*Initialize_ZCL_OTA_Demo*/,          ZCL_OTA_Demo_Create_Server,          ZCL_OTA_Demo_Create_Client},
...
#endif
   { QAPI_ZB_CL_CLUSTER_ID_COLOR_CONTROL,           "COL", Initialize_ZCL_ColorControl_Demo, ZCL_ColorControl_Demo_Create_Server, ZCL_ColorControl_Demo_Create_Client   },
...
const QCLI_Command_t ZigBee_CL_CMD_List[] =
{
   ...
   { cmd_ZB_CL_Help,                false,  "HELP",                "",                                              "Lists the zigbee clusters commands."   },
   ...
   { cmd_ZB_CL_MatchDescriptor,     false,  "MatchDescriptor",     "[NwkAddr][ClusterID][(0=Server,1=Client)]",     "Find an endpoint using a match descriptor request."   },
...
static qbool_t ZCL_InitializeClusters(uint8_t Endpoint, const char *DeviceName, qbool_t ServerList, const uint16_t *ClusterList, uint8_t ClusterCount)
{
   ...
   if (ClusterList != NULL)
   {
      ...
      for (Index = 0; (Index < ClusterCount) && (Ret_Val); Index ++)
      {
         ...
         {
            QCLI_Printf("Cluster Descriptor not found: %d.\n", ClusterList[Index]);
            Ret_Val = false;
         }
      }
   }
...
Parameter_Count is number of elements in Parameter_List.
...
QCLI_Command_Status_t cmd_ZB_CL_CreateEndpoint(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   if (ZigBee_Handle != NULL)
   {
         ...
         (Verify_Integer_Parameter(&Parameter_List[1], ZCL_DEMO_ENDPOINT_TYPE_START_INDEX, ENDPOINT_DECRIPTOR_LIST_SIZE + ZCL_DEMO_ENDPOINT_TYPE_START_INDEX)))
      {
				...
				if (Result == QAPI_OK)
                  {
                     if (EndpointDescriptor->Touchlink)
                     {
                        ...
                        if (Result == QAPI_OK)
                        {
                           ...
                           {
                              QCLI_Printf("qapi_ZB_BDB_Set_Request", Result);
                              Ret_Val = QCLI_STATUS_ERROR_E;
                           }
                        }
                        else
                        {
                           QCLI_Printf("qapi_ZB_BDB_Get_Request", Result);
                           Ret_Val = QCLI_STATUS_ERROR_E;
                        }
                     }
                     else
                     ...
                     {
                        QCLI_Printf("Endpoint %d Initialized as %s.\n", Endpoint, EndpointDescriptor->EndpointName);
                     }
                  }
                  else
                  {
                     QCLI_Printf("qapi_ZB_APS_Add_Endpoint", Result);
                     Ret_Val = QCLI_STATUS_ERROR_E;
                  }
               }
               else
         ...
         {
            QCLI_Printf("Endpoint already exists.\n");
            Ret_Val = QCLI_STATUS_ERROR_E;
         }
      }
      else
   ...
   {
      QCLI_Printf("Zigbee stack is not initialized.\n");
      Ret_Val = QCLI_STATUS_ERROR_E;
   }
...
QCLI_Command_Status_t cmd_ZB_CL_RemoveEndpoint(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   if (ZigBee_Handle != NULL)
   {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[0]), QAPI_ZB_APS_MIN_ENDPOINT, QAPI_ZB_APS_MAX_ENDPOINT)))
      {
         ...
         {
            Ret_Val = QCLI_STATUS_SUCCESS_E;
            QCLI_Printf("qapi_ZB_APS_Remove_Endpoint");
         }
         else
         {
            QCLI_Printf("qapi_ZB_APS_Remove_Endpoint", Result);
            Ret_Val = QCLI_STATUS_ERROR_E;
         }
...
QCLI_Command_Status_t cmd_ZB_CL_ReadLocalAttribute(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   if (GetZigBeeHandle() != NULL)
   {
         ...
         (Verify_Integer_Parameter(&Parameter_List[2], 0, sizeof(AttrValue))))
      {
         ...
         if (ClusterInfo != NULL)
         {
            ...
            switch (Result)
            {
               case QAPI_ERR_BOUNDS:
                  QCLI_Printf("qapi_ZB_CL_Read_Local_Attribute", Result);
                  QCLI_Printf("AttrLength: %d\n", AttrLength);
                  Ret_Val = QCLI_STATUS_USAGE_E;
                  break;

               case QAPI_OK:
                  QCLI_Printf("qapi_ZB_CL_Read_Local_Attribute");
                  QCLI_Printf("   AttrId:     0x%04X\n", AttrId);
                  QCLI_Printf("   AttrLength: %d\n", AttrLength);
                  QCLI_Printf("   AttrValue:  ");
         ...
         {
            QCLI_Printf("Invalid ClusterIndex.\n");
            Ret_Val = QCLI_STATUS_ERROR_E;
         }
      }
      else
         ...
         (Verify_Integer_Parameter(&Parameter_List[2], 1, 0xFF)))
      {
         ...
         if (ClusterInfo != NULL)
         {
            ...
            if (Ret_Val == QCLI_STATUS_SUCCESS_E)
            {
               ...
               if (Result == QAPI_OK)
               {
                  QCLI_Printf("qapi_ZB_CL_Write_Local_Attribute");
               }
               else
               {
                  Ret_Val = QCLI_STATUS_ERROR_E;
                  QCLI_Printf("qapi_ZB_CL_Write_Local_Attribute", Result);
               }
         ...
         (Verify_Integer_Parameter(&(Parameter_List[2]), 0x0000, 0xFFFF)))
      {
         ...
         if (ClusterInfo != NULL)
         {
            ...
            if (Format_Send_Info_By_Device(DeviceId, &SendInfo))
            {
               ...
               if (Result == QAPI_OK)
               {
                  QCLI_Printf("qapi_ZB_CL_Read_Attributes");
               }
               else
               {
                  Ret_Val = QCLI_STATUS_ERROR_E;
                  QCLI_Printf("qapi_ZB_CL_Read_Attributes", Result);
               }
            }
            else
            {
               QCLI_Printf("Invalid device ID.\n");
               Ret_Val = QCLI_STATUS_ERROR_E;
            }
         }
         else
         ...
         if (ClusterInfo != NULL)
         {
            ...
            if (Ret_Val == QCLI_STATUS_SUCCESS_E)
            {
               ...
               if (Format_Send_Info_By_Device(DeviceId, &SendInfo))
               {
                  ...
                  if (Result == QAPI_OK)
                  {
                     QCLI_Printf("qapi_ZB_CL_Write_Attributes");
                  }
                  else
                  {
                     Ret_Val = QCLI_STATUS_ERROR_E;
                     QCLI_Printf("qapi_ZB_CL_Write_Attributes", Result);
                  }
               }
               else
               {
         ...
         (Hex_String_To_ULL(Parameter_List[6].String_Value, &ReportableValueULL)))
      {
         ...
         if (ClusterInfo != NULL)
         {
            ...
            if (Format_Send_Info_By_Device(DeviceId, &SendInfo))
            {
               ...
               if (Result == QAPI_OK)
               {
                  QCLI_Printf("qapi_ZB_CL_Configure_Reporting");
                  Ret_Val = QCLI_STATUS_SUCCESS_E;
               }
               else
               {
                  QCLI_Printf("qapi_ZB_CL_Configure_Reporting", Result);
                  Ret_Val = QCLI_STATUS_ERROR_E;
               }
            }
            else
         ...
         (Verify_Integer_Parameter(&(Parameter_List[2]), 0, 0xFFFF)))
      {
         ...
         if (ClusterInfo != NULL)
         {
            ...
            if (Format_Send_Info_By_Device(DeviceId, &SendInfo))
            {
               ...
               if (Result == QAPI_OK)
               {
                  QCLI_Printf("qapi_ZB_CL_Read_Reporting_Config");
                  Ret_Val = QCLI_STATUS_SUCCESS_E;
               }
               else
               {
                  QCLI_Printf("qapi_ZB_CL_Read_Reporting_Config", Result);
                  Ret_Val = QCLI_STATUS_ERROR_E;
               }
            }
            else
...
QCLI_Command_Status_t cmd_ZB_CL_MatchDescriptor(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   if (ZigBee_Handle != NULL)
   {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[2]), 0, 1)))
      {
         ...
         Result = qc_drv_ZB_ZDP_Match_Desc_Req(qc_api_get_qc_drv_context(), ZigBee_Handle, NWKAddr, &Match_Request);
         if (Result == QAPI_OK)
         {
            QCLI_Printf("Finding Cluster 0x%04X.\n", ClusterId);
            Ret_Val =  QCLI_STATUS_SUCCESS_E;
         }
         else
         {
            QCLI_Printf("qapi_ZB_ZDP_Match_Desc_Req", Result);
            Ret_Val = QCLI_STATUS_ERROR_E;
         }
      }
      else
   ...
   if (GetZigBeeHandle() != NULL)
   {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[1]), 0, ZCL_Demo_Context.Cluster_Count - 1)))
      {
         if (ZCL_Demo_Context.DiscoverAttr_NextId == 0)
         {
            ...
            if (ClusterInfo != NULL)
            {
               ...
               if (Format_Send_Info_By_Device(DeviceId, &SendInfo))
               {
                  ...
                  if (Result == QAPI_OK)
                  {
                     ...
                     QCLI_Printf("qapi_ZB_CL_Discover_Attributes");
                  }
                  else
                  {
                     Ret_Val = QCLI_STATUS_ERROR_E;
                     QCLI_Printf("qapi_ZB_CL_Discover_Attributes", Result);
                  }
               }
               else
               {
            ...
            {
               QCLI_Printf("Demo only supports one attribute discovery at a time.\n");
               Ret_Val = QCLI_STATUS_ERROR_E;
            }
         }
         else
   ...
   if ((ZB_Handle == GetZigBeeHandle()) && (Event_Data != NULL))
   {
      switch (Event_Data->Event_Type)
      {
         ...
         case QAPI_ZB_CL_EVENT_TYPE_CONFIG_REPORT_RESPONSE_E:
            QCLI_Printf("Config Report Response:\n");
            DisplayGeneralReceiveInfo(&(Event_Data->Receive_Info));
         ...
         case QAPI_ZB_CL_EVENT_TYPE_READ_REPORT_CONFIG_RESPONSE_E:
            QCLI_Printf("Read Report Config Response:\n");
            DisplayGeneralReceiveInfo(&(Event_Data->Receive_Info));
...
qbool_t Initialize_ZCL_Demo(QCLI_Group_Handle_t ZigBee_QCLI_Handle)
{
   ...
   {
      QCLI_Printf("Failed to register ZigBee Cluster Library command group.\n");
      Ret_Val = false;
   }
...
int16_t ZB_Cluster_AddCluster(const ZCL_Demo_Cluster_Info_t *Cluster_Info)
{
   ...
   if ((Cluster_Info != NULL) && (Cluster_Info->ClusterName != NULL) && (Cluster_Info->DeviceName != NULL) && ((Cluster_Info->ClusterType == ZCL_DEMO_CLUSTERTYPE_CLIENT) || (Cluster_Info->ClusterType == ZCL_DEMO_CLUSTERTYPE_SERVER)))
   {
      if (ZCL_Demo_Context.Cluster_Count < CLUSTER_LIST_SIZE)
      {
         ...
         if (!(ZCL_Demo_Context.ZCL_CB_Registered))
         {
            ...
            if (ZigBee_Handle != NULL)
            {
               ...
               if (Result == QAPI_OK)
               {
                  QCLI_Printf("qapi_ZB_CL_Register_Callback");
                  ZCL_Demo_Context.ZCL_CB_Registered = true;
               }
               else
               {
                  QCLI_Printf("qapi_ZB_CL_Register_Callback", Result);
               }
            }
         }
      }
      ...
      {
         QCLI_Printf("Cluster list full.\n");
         Ret_Val = -1;
      }
   }
   else
   {
      QCLI_Printf("ZB_Cluster_AddCluster: Invalid parameter.\n");
      Ret_Val = -1;
   }
