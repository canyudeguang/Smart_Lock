...
/* Function prototypes. */
static QCLI_Command_Status_t cmd_ZCL_Custom_Help(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List);
...
static const QCLI_Command_t ZigBee_Custom_CMD_List[] =
{
   ...
   {cmd_ZCL_Custom_Help,        false,  "Help",        "",                        "Display the available ZCL custom commands"},
         ...
         (Verify_Integer_Parameter(&(Parameter_List[1]), QAPI_ZB_APS_MIN_ENDPOINT, QAPI_ZB_APS_MAX_ENDPOINT)))
      {
         ...
         if(ClusterInfo != NULL)
         {
            ...
            if(Format_Send_Info_By_Device(DeviceId, &SendInfo))
            {
               ...
               if(Result == QAPI_OK)
               {
                  LOG_INFO("qapi_ZB_CL_Send_Command");
               }
               else
               {
                  Ret_Val = QCLI_STATUS_ERROR_E;
                  LOG_ERR("qapi_ZB_CL_Send_Command", Result);
               }
            }
            else
            {
   ...
   if((ZB_Handle != NULL) && (Cluster != NULL) && (EventData != NULL))
   {
      ...
      switch(EventData->Event_Type)
      {
         case QAPI_ZB_CL_CUSTOM_CLUSTER_EVENT_TYPE_UNPARSED_DATA_E:
            LOG_AT_EVT("EVT_ZB: Custom Cluster Command %d:\n", EventData->Data.Unparsed_Data.Header.CommandId);
            Dump_Data("  ", EventData->Data.Unparsed_Data.APSDEData.ASDULength, EventData->Data.Unparsed_Data.APSDEData.ASDU);
            *(EventData->Data.Unparsed_Data.Result) = QAPI_OK;
            ...
            if(Result != QAPI_OK)
            {
               LOG_AT_EVT("EVT_ZB: qapi_ZB_CL_Default_Response", Result);
            }
            break;

         case QAPI_ZB_CL_CUSTOM_CLUSTER_EVENT_TYPE_ALARM_RESET_E:
            LOG_AT_EVT("EVT_ZB: Reset Alarm:\n");
            LOG_AT_EVT("EVT_ZB: Cluster ID: 0x%04X\n", EventData->Data.Reset_Alarm_Data.ClusterId);
            LOG_AT_EVT("EVT_ZB: Alarm Code: %u\n", EventData->Data.Reset_Alarm_Data.AlarmCode);
            break;
         ...
         default:
            LOG_AT_EVT("EVT_ZB: Unhandled Custom server event %d.\n", EventData->Event_Type);
            break;
      }
   ...
   if((ZB_Handle != NULL) && (Cluster != NULL) && (EventData != NULL))
   {
      ...
      switch(EventData->Event_Type)
      {
         case QAPI_ZB_CL_CUSTOM_CLUSTER_EVENT_TYPE_DEFAULT_RESPONSE_E:
            LOG_AT_EVT("EVT_ZB: Custom Client Default Response:\n");
         ...
         case QAPI_ZB_CL_CUSTOM_CLUSTER_EVENT_TYPE_UNPARSED_RESPONSE_E:
            LOG_AT_EVT("EVT_ZB: Unparsed client response: %d.\n", EventData->Event_Type);
            break;

         default:
            LOG_AT_EVT("EVT_ZB: Unhandled Custom client event %d.\n", EventData->Event_Type);
            break;
      }
...
qbool_t Initialize_ZCL_Custom_Demo(QCLI_Group_Handle_t ZigBee_QCLI_Handle)
{
   ...
   {
      LOG_WARN("Failed to register ZCL Custom command group.\n");
      Ret_Val = false;
   }
