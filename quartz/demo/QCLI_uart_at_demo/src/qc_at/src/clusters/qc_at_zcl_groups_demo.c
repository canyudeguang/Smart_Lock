...
const QCLI_Command_t ZigBee_Groups_CMD_List[] =
{
   ...
   {cmd_ZCL_Groups_Help,               false,  "Help",               "",                                                    "Display the ZCL groups commands."},
   {cmd_ZCL_Groups_AddGroup,           false,  "add",           "[DevId][ClientEndpoint][GroupID][Name][Identifying]", "Sends an Add Groups command to a Groups server."},
   {cmd_ZCL_Groups_ViewGroup,          false,  "view",          "[DevId][ClientEndpoint][GroupID]",                    "Sends a View Groups command to a Groups server."},
   {cmd_ZCL_Groups_GetGroupMembership, false,  "GroupMem", "[DevId][ClientEndpoint][GroupID (Optional)]",         "Sends a Get Group Membership command to a Groups server."},
   {cmd_ZCL_Groups_RemoveGroup,        false,  "remove",        "[DevId][ClientEndpoint][GroupID]",                    "Sends a Remove Group command to a Groups server."},
   {cmd_ZCL_Groups_RemoveAllGroups,    false,  "RemAll",    "[DevId][ClientEndpoint]",                             "Sends a Remove All Groups command to a Groups server."}
};
         ...
         (Verify_Integer_Parameter(&(Parameter_List[4]), 0, 1)))
      {
         ...
         if(ClusterInfo != NULL)
         {
            ...
            if(Format_Send_Info_By_Device(DeviceId, &SendInfo))
            {
               ...
               if(Result == QAPI_OK)
               {
                  LOG_INFO("qc_drv_ZB_CL_Groups_Send_Add_Group");
               }
               else
               {
                  Ret_Val = QCLI_STATUS_ERROR_E;
                  LOG_ERR("qc_drv_ZB_CL_Groups_Send_Add_Group", Result);
               }
            }
            else
            {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[2]), 0, 0xFFFF)))
      {
         ...
         if(ClusterInfo != NULL)
         {
            ...
            if(Format_Send_Info_By_Device(DeviceId, &SendInfo))
            {
               ...
               if(Result == QAPI_OK)
               {
                  LOG_INFO("qc_drv_ZB_CL_Groups_Send_View_Group");
               }
               else
               {
                  Ret_Val = QCLI_STATUS_ERROR_E;
                  LOG_ERR("qc_drv_ZB_CL_Groups_Send_View_Group", Result);
               }
            }
            else
            {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[1]), QAPI_ZB_APS_MIN_ENDPOINT, QAPI_ZB_APS_MAX_ENDPOINT)))
      {
         ...
         if(ClusterInfo != NULL)
         {
            ...
            if(Format_Send_Info_By_Device(DeviceId, &SendInfo))
            {
               ...
               if(Result == QAPI_OK)
               {
                  LOG_INFO("qc_drv_ZB_CL_Groups_Send_Get_Group_Membership");
               }
               else
               {
                  Ret_Val = QCLI_STATUS_ERROR_E;
                  LOG_ERR("qc_drv_ZB_CL_Groups_Send_Get_Group_Membership", Result);
               }
            }
            else
            {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[2]), 0, 0xFFFF)))
      {
         ...
         if(ClusterInfo != NULL)
         {
            ...
            if(Format_Send_Info_By_Device(DeviceId, &SendInfo))
            {
               ...
               if(Result == QAPI_OK)
               {
                  LOG_INFO("qc_drv_ZB_CL_Groups_Send_Remove_Group");
               }
               else
               {
                  Ret_Val = QCLI_STATUS_ERROR_E;
                  LOG_ERR("qc_drv_ZB_CL_Groups_Send_Remove_Group", Result);
               }
            }
            else
            {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[1]), QAPI_ZB_APS_MIN_ENDPOINT, QAPI_ZB_APS_MAX_ENDPOINT)))
      {
         ...
         if(ClusterInfo != NULL)
         {
            ...
            if(Format_Send_Info_By_Device(DeviceId, &SendInfo))
            {
               ...
               if(Result == QAPI_OK)
               {
                  LOG_INFO("qc_drv_ZB_CL_Groups_Send_Remove_All_Groups");
               }
               else
               {
                  Ret_Val = QCLI_STATUS_ERROR_E;
                  LOG_ERR("qc_drv_ZB_CL_Groups_Send_Remove_All_Groups", Result);
               }
            }
            else
            {
...
static void ZCL_Groups_Demo_Server_CB(qapi_ZB_Handle_t ZB_Handle, qapi_ZB_Cluster_t Cluster, qapi_ZB_CL_Groups_Server_Event_Data_t *EventData, uint32_t CB_Param)
{
   ...
   {
      LOG_AT_EVT("EVT_ZB: Unhandled Groups server event %d.\n", EventData->Event_Type);
      QCLI_Display_Prompt();
   }
}
   ...
   if((ZB_Handle != NULL) && (Cluster != NULL) && (EventData != NULL))
   {
      switch(EventData->Event_Type)
      {
         ...
         case QAPI_ZB_CL_GROUPS_CLIENT_EVENT_TYPE_ADD_GROUP_RESPONSE_E:
            LOG_AT_EVT("EVT_ZB: Add Group Response:\n");
         ...
         case QAPI_ZB_CL_GROUPS_CLIENT_EVENT_TYPE_REMOVE_GROUP_RESPONSE_E:
            LOG_AT_EVT("EVT_ZB: Remove Group Response:\n");
            LOG_AT_EVT("EVT_ZB: Status:  %d\n", EventData->Data.Remove_Group_Response.Status);
            LOG_AT_EVT("EVT_ZB: GroupId: %d\n", EventData->Data.Remove_Group_Response.GroupId);
            break;

         default:
            LOG_AT_EVT("EVT_ZB: Unhandled Groups client event %d.\n", EventData->Event_Type);
            break;
      }
      QCLI_Display_Prompt();
   }
...
qbool_t Initialize_ZCL_Groups_Demo(QCLI_Group_Handle_t ZigBee_QCLI_Handle)
{
   ...
   {
      LOG_WARN("Failed to register ZCL Groups command group.\n");
      Ret_Val = false;
   }
...
qapi_ZB_Cluster_t ZCL_Groups_Demo_Create_Server(uint8_t Endpoint, void **PrivData)
{
   ...
   if(ZigBee_Handle != NULL)
   {
      ...
      if(IdentifyClusterInfo != NULL)
      {
         ...
         Result = qapi_ZB_CL_Groups_Populate_Attributes(true, &(ClusterInfo.AttributeCount), (qapi_ZB_CL_Attribute_t*) ClusterInfo.AttributeList);
         if(Result == QAPI_OK)
         {
            ...
            if(Result != QAPI_OK)
            {
               LOG_ERR("qapi_ZB_CL_Groups_Create_Server", Result);
               Ret_Val = NULL;
            }
         }
         else
         {
            LOG_ERR("qapi_ZB_CL_Groups_Populate_Attributes", Result);
            Ret_Val = NULL;
         }
      }
      else
...
qapi_ZB_Cluster_t ZCL_Groups_Demo_Create_Client(uint8_t Endpoint, void **PrivData)
{
   ...
   if(ZigBee_Handle != NULL)
   {
      ...
      if(Result != QAPI_OK)
      {
         LOG_ERR("qapi_ZB_CL_Groups_Create_Client", Result);
         Ret_Val = NULL;
      }
   }
   else
