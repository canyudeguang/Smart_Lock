...
/* Function prototypes. */
static QCLI_Command_Status_t cmd_ZB_OTA_Help(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List);
...
const QCLI_Command_t ZigBee_OTA_CMD_List[] =
{
   ...
   {cmd_ZB_OTA_Help,           false, "Help",           "",                                     "Display the all ZCL ota commands."},
   ...
   if(GetZigBeeHandle() != NULL)
   {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[1]), QAPI_ZB_APS_MIN_ENDPOINT, QAPI_ZB_APS_MAX_ENDPOINT)))
      {
         ...
         if(ClusterInfo != NULL)
         {
            ...
            {
               Ret_Val = QCLI_STATUS_SUCCESS_E;
               LOG_INFO("qapi_ZB_CL_OTA_Client_Discover");
            }
            else
            {
               Ret_Val = QCLI_STATUS_ERROR_E;
               LOG_ERR("qapi_ZB_CL_OTA_Client_Discover", Result);
            }
         }
         else
         {
   ...
   if(GetZigBeeHandle() != NULL)
   {
         ...
         (Parameter_List[1].Integer_Is_Valid) && (Parameter_List[2].Integer_Is_Valid))
      {
         ...
         if(ClusterInfo != NULL)
         {
            ...
            {
               Ret_Val = QCLI_STATUS_SUCCESS_E;
               LOG_INFO("qapi_ZB_CL_OTA_Query_Next_Image");
            }
            else
            {
               Ret_Val = QCLI_STATUS_ERROR_E;
               LOG_ERR("qapi_ZB_CL_OTA_Query_Next_Image", Result);
            }
         }
         else
         {
   ...
   if(GetZigBeeHandle() != NULL)
   {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[0]), QAPI_ZB_APS_MIN_ENDPOINT, QAPI_ZB_APS_MAX_ENDPOINT)))
      {
         ...
         if(ClusterInfo != NULL)
         {
            ...
            {
               Ret_Val = QCLI_STATUS_SUCCESS_E;
               LOG_INFO("qapi_ZB_CL_OTA_Image_Block_Start_Transfer");
            }
            else
            {
               Ret_Val = QCLI_STATUS_ERROR_E;
               LOG_ERR("qapi_ZB_CL_OTA_Image_Block_Start_Transfer", Result);
            }
         }
         else
         {
      ...
      switch(Event_Data->Event_Type)
      {
         case QAPI_ZB_CL_OTA_SERVER_EVENT_TYPE_IMAGE_EVAL_E:
            ...
            *Event_Data->Data.Image_Eval.EvalStatus = UpgradeEligible;
            ...
            if(UpgradeEligible)
            {
               LOG_AT_EVT("EVT_ZB: OTA server image eval callback received, upgrade image available, size %d.\n", (ZIGBEE_OTA_DEMO_PREPEND_SIZE + FileStat.st_size));
               QCLI_Display_Prompt();
            }
            else
            {
               LOG_AT_EVT("EVT_ZB: OTA server image eval callback received, no upgrade image available.\n");
               QCLI_Display_Prompt();
            }
            break;
         case QAPI_ZB_CL_OTA_SERVER_EVENT_TYPE_IMAGE_READ_E:
            LOG_AT_EVT("EVT_ZB: OTA server image read callback received.\n");
            ImgDescriptor = NULL;
            for(Index = 0; Index < sizeof(ZigbeeOTADemoImageDescriptorList)/sizeof(Zigbee_OTA_Demo_Image_Descriptor_t); Index++)
            {
               if(ZigbeeOTADemoImageDescriptorList[Index].ImageType == Event_Data->Data.Image_Eval.ImageDefinition.ImageType)
            ...
            if(ImgDescriptor)
            {
               ...
               if(DataLen)
               {
                  ...
                  if((ImgDescriptor->ImageFd) || ((!ImgDescriptor->ImageFd) && (qapi_Fs_Open(ImgDescriptor->FilePath, QAPI_FS_O_RDONLY, &ImgDescriptor->ImageFd) == QAPI_OK)))
                  {
               ...
               {
                  LOG_AT_EVT("EVT_ZB: Read %d bytes, offset %d.\n", Event_Data->Data.Image_Read.ImageData.DataSize, Event_Data->Data.Image_Read.ImageData.FileOffset);
               }
               else
               {
                  Event_Data->Data.Image_Read.ImageData.DataSize = 0;
                  LOG_AT_EVT("EVT_ZB: Read error.\n");
               }
            }
            QCLI_Display_Prompt();
            break;
         case QAPI_ZB_CL_OTA_SERVER_EVENT_TYPE_IMAGE_UPGRADE_END_ERROR_E:
            LOG_AT_EVT("EVT_ZB: OTA server upgrade end error callback received.\n");
            QCLI_Display_Prompt();
            break;
         case QAPI_ZB_CL_OTA_SERVER_EVENT_TYPE_IMAGE_UPGRADE_END_TIME_E:
            LOG_AT_EVT("EVT_ZB: OTA server upgrade end time callback received.\n");
            QCLI_Display_Prompt();
            break;
         default:
            LOG_AT_EVT("EVT_ZB: Unhandled OTA server event %d.\n", Event_Data->Event_Type);
            break;
      }
   }
}
         ...
         switch(Event_Data->Event_Type)
         {
            case QAPI_ZB_CL_OTA_CLIENT_EVENT_TYPE_QUERY_NEXT_IMAGE_E:
               if(Event_Data->Data.Query_Next_Image.Status == QAPI_OK)
                  LOG_AT_EVT("EVT_ZB: OTA client query next image callback received, status %d, size %d.\n", Event_Data->Data.Query_Next_Image.Status, Event_Data->Data.Query_Next_Image.ImageSize);
               else
                  LOG_AT_EVT("EVT_ZB: OTA client query next image callback received, status %d.\n", Event_Data->Data.Query_Next_Image.Status);
               QCLI_Display_Prompt();
               break;
...
qbool_t Initialize_ZCL_OTA_Demo(QCLI_Group_Handle_t ZigBee_QCLI_Handle)
{
   ...
   {
      LOG_WARN("Failed to register ZCL OTA command group.\n");
      Ret_Val = false;
   }
...
qapi_ZB_Cluster_t ZCL_OTA_Demo_Create_Server(uint8_t Endpoint, void **PrivData)
{
   ...
   if(ZigBee_Handle != NULL)
   {
      ...
      Result = qapi_ZB_CL_OTA_Populate_Attributes(true, &(ClusterInfo.AttributeCount), (qapi_ZB_CL_Attribute_t*) ClusterInfo.AttributeList);
      if(Result == QAPI_OK)
      {
         ...
         {
            LOG_ERR("qapi_ZB_CL_OTA_Create_Server", Result);
            Ret_Val = NULL;
         }
      }
      else
      {
         LOG_ERR("qapi_ZB_CL_OTA_Populate_Attributes", Result);
         Ret_Val = NULL;
      }
   }
   else
...
qapi_ZB_Cluster_t ZCL_OTA_Demo_Create_Client(uint8_t Endpoint, void **PrivData)
{
   ...
   if(ZigBee_Handle != NULL)
   {
      ...
      /* Setup the ClusterInfo for the OTA client cluster. */
      Result = qapi_ZB_CL_OTA_Populate_Attributes(true, &(ClusterInfo.AttributeCount), (qapi_ZB_CL_Attribute_t*) ClusterInfo.AttributeList);
      if(Result == QAPI_OK)
      {
         ...
         if((Result != QAPI_OK) || (Ret_Val == NULL))
         {
            LOG_ERR("qapi_ZB_CL_OTA_Create_Client", Result);
            Ret_Val = NULL;
         }
      }
      else
