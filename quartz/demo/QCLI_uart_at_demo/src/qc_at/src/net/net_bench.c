	...
	switch(pattern_type) {
		case 1:
			if (Parameter_Count == 2) {
				QCLI_Printf("Missing arguments, Provide the pattern. For eg: 0xAA\n");
			}
			else {
				pattern = Parameter_List[2].Integer_Value;
				QCLI_Printf("Pattern set to %x\n", pattern);
			}
			break;
		case 2:
			QCLI_Printf("Pattern type set to ASCII chars(0x20-0x7F)\n");
			break;
		default:
			QCLI_Printf("Pattern type is default(continuous) 00..FF\n");
			break;
	}
...
void bench_common_print_test_results(THROUGHPUT_CXT *p_tCxt, STATS *pktStats)
{
    ...
    if (last_tick < first_tick)
    {
        /* Assume the systick wraps around once */
        duration = ~first_tick + 1 + last_tick;
        QCLI_Printf("last: 0x%x first: 0x%x duration: 0x%x\n",
    ...
    QCLI_Printf("\t%llu KBytes %llu bytes (%llu bytes) in %u seconds %u ms (%llu miliseconds)\n\n",
            total_bytes/1024, total_bytes%1024, total_bytes, sec_interval, (uint32_t)(total_interval%1000), total_interval);
    QCLI_Printf("\tThroughput: %u Kbits/sec\n", throughput);
}
...
static void tx_help(uint32_t v6)
{
    if (v6)
    {
        QCLI_Printf("benchtx6 <Rx IP> <port> {tcp|tcpzc|udp|udpzc|ssl} <msg size> <mode> <arg> <delay in microseconds between msgs> [<tclass>]\n");
        QCLI_Printf(" <mode> can be 0 or 1.\n");
        QCLI_Printf(" If <mode> is 0, <arg> is time to TX in seconds.\n");
        QCLI_Printf(" If <mode> is 1, <arg> is number of msgs to TX.\n");
        QCLI_Printf("Examples:\n");
        QCLI_Printf(" benchtx6 fe80::865d:d7ff:fe40:3498%%wlan1 2390 udp 1400 1 100 0 0xA0\n");
        QCLI_Printf(" benchtx6 2001:db8:85a3:8d3:1319:8a2e:370:734 2390 tcpzc 512 0 30 5\n");
    }
    else
    {
        QCLI_Printf("benchtx <Rx IP> <port> {tcp|tcpzc|udp|udpzc|ssl} <msg size> <mode> <arg> <delay in microseconds between msgs> [<tos>] <source IP>\n");
        QCLI_Printf("benchtx <Rx IP> <protocol> raw <msg size> <mode> <arg> <delay in microseconds between msgs> [<tos>]\n");
        QCLI_Printf("benchtx <Rx IP> <protocol> rawh <msg size> <mode> <arg> <delay in microseconds between msgs> <tos> <source IP>\n");
        QCLI_Printf(" <mode> can be 0 or 1.\n");
        QCLI_Printf(" If <mode> is 0, <arg> is time to TX in seconds.\n");
        QCLI_Printf(" If <mode> is 1, <arg> is number of msgs to TX.\n");
        QCLI_Printf("Examples:\n");
        QCLI_Printf(" benchtx 192.168.1.20 2390 udp 1400 1 100 0 0xA0\n");
        QCLI_Printf(" benchtx 255.255.255.255 5001 udp 1200 0 30 0 0xA0 192.168.1.145\n");
        QCLI_Printf(" benchtx 192.168.1.20 26 raw 1400 0 60 0\n");
        QCLI_Printf(" benchtx 192.168.1.20 26 rawh 1400 1 100 10 0xA0 192.168.1.100\n");
    }
}
...
uint32_t bench_common_SetParams(THROUGHPUT_CXT *p_rxtCxt, uint32_t v6, const char *protocol, uint16_t port, enum test_type type)
{
	...
	if (type == RX) {
		if (bench_common_IsPortInUse(p_rxtCxt, port)) {
			QCLI_Printf("port %d is in use; use another port.\n", port);
...
char* bench_common_GetInterfaceNameFromStr(char *ipstr)
{
	...
	if ( interface_name_with_percent_char ) {
		...
		QCLI_Printf("Multicast IPv6 address is used, must append %%interface_name to the address\n");
		QCLI_Printf("For example: ff02::1%%wlan1 if you intend to listen on a multicast address on interface wlan1\n");
	}
...
static QCLI_Command_Status_t bench_common_tx(THROUGHPUT_CXT *p_tCxt, uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
    ...
    {
        QCLI_Printf("ERROR: packet size is 0\n");
        return QCLI_STATUS_ERROR_E;
    }
    ...
    {
        QCLI_Printf("Invalid test mode, please enter 0 for duration test or 1 for packet test\n");
        return QCLI_STATUS_ERROR_E;
    }
    ...
    switch(p_tCxt->protocol) {
		...
		case UDP:
   		...
   		if (Parameter_Count == 8 && p_tCxt->params.tx_params.ip_address == 0xFFFFFFFF) {
   			QCLI_Printf("If dest is broadcast, please specify the src ip\n");
			e = -1;
			goto end;
   		}
   		...
   		if (Parameter_Count == 9) {
			...
			{
				QCLI_Printf("Incorrect address %s\n", Parameter_List[8].String_Value);
				e = -1;
				goto end;
			}
   		}
	...
	break;
        ...
        {
            QCLI_Printf("ERROR: SSL client not started (Use 'ssl start client' first).\n");
            e = -1;
            goto end;
        }
	...
	break;
   default:
        QCLI_Printf("Invalid protocol: %s\n", protocol);
        e = -1;
		break;
	}
...
QCLI_Command_Status_t bench_common_tx6(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
    ...
    {
        QCLI_Printf("Incorrect IPv6 address %s\n", receiver_ip);
        return QCLI_STATUS_ERROR_E;
    }
    		...
    		QAPI_IS_IPV6_MULTICAST(tCxt.params.tx_params.v6addr))
    {
        ...
        if ( retval != 0 ) {
            QCLI_Printf("Failed to get scope id for the interface %s\n", interface_name);
            return QCLI_STATUS_ERROR_E;
        }
    }
...
static void rx_help(uint32_t v6)
{
    if (v6)
    {
        QCLI_Printf("benchrx6 {udp|udpzc|udpecho|tcp|tcpzc|tcpecho|ssl|sslzc} <port>\n");
        QCLI_Printf("benchrx6 {udp|udpzc} <port> [<multicast_ip%%if_name>]\n");
        QCLI_Printf("Examples:\n");
        QCLI_Printf(" benchrx6 tcp 2390\n");
        QCLI_Printf(" benchrx6 udp 2390 ff02::1:2%%wlan1\n");
    }
    else
    {
        QCLI_Printf("benchrx {tcp|tcpzc|tcpecho|ssl|sslzc} <port> [<local_ip>]\n");
        QCLI_Printf("benchrx {udp|udpzc|udpecho} <port> [<local_ip>|<multicast_ip>|<multicast_ip%%if_name>|<multicast_ip> <local_ip>]\n");
        QCLI_Printf("benchrx {raw|rawh} <protocol> [<local_ip>|<multicast_ip>|<multicast_ip%%if_name>|<multicast_ip> <local_ip>]\n");
        QCLI_Printf("Examples:\n");
        QCLI_Printf(" benchrx tcp 2390\n");
        QCLI_Printf(" benchrx tcpzc 5000 192.168.1.100\n");
        QCLI_Printf(" benchrx udp 6000 224.1.1.100%%wlan1\n");
        QCLI_Printf(" benchrx udp 7000 224.1.1.100 192.168.1.100\n");
        QCLI_Printf(" benchrx rawh 26\n");
    }
}
...
int bench_common_SetMCParams(THROUGHPUT_CXT *p_rxtCxt, char *remoteIP, char *localIP, int v6)
 {
	if (v6) {
		...
		if (inet_pton(AF_INET6, remoteIP, p_rxtCxt->params.rx_params.mcIpv6addr)
			...
			QCLI_Printf("Incorrect IPv6 multicast address %s\n", remoteIP);
			return -1;
		}
		...
		if (localIP) {
			...
			!QAPI_IPV4_IS_MULTICAST(p_rxtCxt->params.rx_params.mcIpaddr)) {
				QCLI_Printf("Incorrect IPv4 multicast address %s\n", remoteIP);
				return -1;
			}
			...
			if (strchr(temp_address_str, '%')) {
				...
				if (QAPI_IPV4_IS_MULTICAST(address)) {
					...
					if (qapi_Net_IPv4_Config(interface_name,
						...
						QCLI_Printf("Invalid interface name %s\n", interface_name);
						return -2;
					}
					...
					QCLI_Printf("Invalid Multicast IP %s\n",
							address_str);
					return -4;
				}
			}
...
unsigned short ratio(uint32_t numerator, uint32_t denominator, unsigned short base)
{
    ...
    if (base) {
        if (denominator) {
            ...
            QCLI_Printf("warning, denominator=%d\n", denominator);
        }
    } else {
        QCLI_Printf("warning, base=%d not supported\n", base);
    }
    return ret;
}
...
void send_ack(THROUGHPUT_CXT *p_tCxt, struct sockaddr *faddr, int addrlen)
{
    ...
    while (retry)
    {
        ...
        if (p_tCxt->protocol == SSL && p_tCxt->test_type == RX)
        {
            ...
            if(send_result == QAPI_ERR_SSL_CONN_NOT_FOUND) {
            	QCLI_Printf("SSL connection closed by peer\n");
            	retry = 0;
            	break;
            }
        }
        ...
        {
            QCLI_Printf("Error while sending stat packet, e=%d\n", send_result);
            app_msec_delay(1);
        }
        else /* Sending ACK is successful.  Now waiting 1000 ms for more endmark packets from client. */
        {
            ...
            if (conn_sock > 0)
            {
                ...
                if (received > 0)
                {
                    QCLI_Printf("ACK success (%u)\n", MAX_ACK_RETRY - retry);
                    break;
                }
                QCLI_Printf(" received %d\n", received);
            }
            else
            {
    ...
    if (retry == 0)
    {
        QCLI_Printf("Did not receive ACK.\n");
    }
...
void send_ack_zc(THROUGHPUT_CXT *p_tCxt, struct sockaddr *faddr, uint32_t addrlen)
{
    ...
    while (retry)
    {
        ...
        if ( result == A_ERROR )
        {
            QCLI_Printf("error while sending stat packet\n");
            break;
        }
        else /* Sending ACK is successful.  Now waiting 1000 ms for more endmark packets from client. */
        {
...
void bench_print_spaces_internal(uint32_t spaces)
{
	uint32_t i;
...
void bench_print_ascii_internal(const char *buf, uint32_t len)
{
	uint32_t i;
...
//    QCLI_Group_Handle_t handle = qcli_net_handle;
    ...
    if(!buf)
    {
    	QCLI_Printf("Buffer error\n");
    	return;
    }

/*    if(!handle)
    {
    	QCLI_Printf("Handle error\n");
    	return;
    }
*/
	...
	for (i = 0; i < len; i++)
	{
		...
		{
			bench_print_spaces_internal(spaces);
			bench_print_ascii_internal(&buf[print_offset], PRINT_ELEMENTS_PER_LINE);
	...
	{
		bench_print_spaces_internal(spaces);
		bench_print_ascii_internal(buf + print_offset, PRINT_ELEMENTS_PER_LINE - j);
	}
...
QCLI_Command_Status_t queuecfg(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
    ...
    if (strcasecmp("rx", Parameter_List[0].String_Value) == 0)
    {
        ...
        {
            rxqueue_size = -1;
            QCLI_Printf("RX queue size: DEFAULT\n");
        }
        else
        {
            QCLI_Printf("RX queue size: %d\n", rxqueue_size);
        }
    }
    else if (strcasecmp("tx", Parameter_List[0].String_Value) == 0)
    {
        ...
        {
            txqueue_size = -1;
            QCLI_Printf("TX queue size: DEFAULT\n");
        }
        else
        {
            QCLI_Printf("TX queue size: %d\n", txqueue_size);
        }
    }
    else
    {
    ...
    QCLI_Printf("Restart benchmarking threads to apply changes.\n");
    return QCLI_STATUS_SUCCESS_E;
}
