    ...
    (stat_udp_pattern_t *stat_udp)
{
    ...
    QCLI_Printf("udp pkts: plan=%d recvd=%d drop_ratio=%d/%d\n",
        stat_udp->pkts_plan, stat_udp->pkts_recvd, stat_udp->ratio_of_drop, RATIO_BASE);
    QCLI_Printf("udp pkts of seq: recvd=%d less=%d less_ratio=%d/%d\n",
    ...
    if (pkt)
    {
        ...
        if (family == AF_INET)
        {
...
#ifdef ZCOPY_UDP_RX_DEBUG
            {
                ...
                QCLI_Printf("%s: pkt=0x%p from %s:%u tlen=%u plen=%u pk_next=0x%p so=0x%p\n", __func__, pkt,
        ...
        else
        {
...
#ifdef ZCOPY_UDP_RX_DEBUG
            {
                ...
                QCLI_Printf("%s: pkt=0x%p from %s:%u tlen=%u plen=%u pk_next=0x%p s=0x%p\n", __func__, pkt,
                        inet_ntop(family, &from->sin_addr, ip_str, sizeof(ip_str)), ntohs(from->sin_port),
                        pp->nb_tlen, pp->nb_plen, pp->pk_next, so);
            }
#endif
...
int bench_udp_open(THROUGHPUT_CXT *p_tCxt)
{
    ...
    if (p_tCxt->params.rx_params.mcEnabled)
    {
        if (p_tCxt->params.rx_params.v6)
        {
            ...
            {
                QCLI_Printf("ERROR: Socket set option failure.\n");
                qapi_socketclose(p_tCxt->sock_local);
				return -3;
            }
        }
...
void bench_udp_rx(THROUGHPUT_CXT *p_tCxt)
{
    ...
    if (p_tCxt->is_iperf)
    {
        ...
        QCLI_Printf("------------------------------------------------------------\n");
        QCLI_Printf("Server listening on UDP port %d\n", port);
        QCLI_Printf("UDP buffer size:  %d\n",buffer_size);
        QCLI_Printf("------------------------------------------------------------\n");
    }
    else
    {
    ...
    while (!benchrx_quit) /* Main loop */
    {
        ...
        while (!benchrx_quit)   /* Receive loop */
        {
                ...
                if (p_tCxt->is_iperf)
                {
                    if (p_tCxt->buffer[0] == 0xFF)
                    {
                        ...
                        while (retry)
                        {
                            ...
                            {
                                QCLI_Printf("%d error while sending ack packet, e=%d\n", retry, send_result);
                    ...
                    ((EOT_PACKET *)p_tCxt->buffer)->code != (int)END_OF_TEST_CODE)
                {
                    ...
                    if (is_first)
                    {
                        ...
                        if (p_tCxt->test_type == RX && p_tCxt->protocol == SSL) {
                            ...
                            if (alpn_protocol) {
                                QCLI_Printf("Negotiated Application Protocol: %s\n", alpn_protocol);
                            }
						}
#endif
...
#ifdef UDP_RX_TS_DEBUG
                    QCLI_Printf("last_time: sec=%u ms=%u rcvd=%u\n",
            ...
            {
                QCLI_Printf("%d received= %d\n", i, received);
                break;
            }
        } /* receive_loop */

ERROR_3:
        QCLI_Printf("Received %llu bytes from %s:%d\n",
                p_tCxt->pktStats.bytes, ip_str,
                ntohs(from->sa_port));
...
void bench_udp_rx_zc(THROUGHPUT_CXT *p_tCxt)
{
    ...
    /* ------ Start test.----------- */
    QCLI_Printf("****************************************************\n");
    QCLI_Printf("IOT %s Zero-Copy RX Test\n", (p_tCxt->test_type == RX && p_tCxt->protocol == SSL) ? "SSL" : "UDP");
    QCLI_Printf("****************************************************\n");
    QCLI_Printf("Bind address %s\n", inet_ntop(family, local_sin_addr, ip_str, sizeof(ip_str)));
    QCLI_Printf("Local port %d\n", port);
    QCLI_Printf("Type benchquit to terminate test\n");
    ...
    if(!(p_tCxt->test_type == RX && p_tCxt->protocol == SSL))
    {
        ...
        {
            QCLI_Printf("ERROR: Socket callback\n");
            goto ERROR_2;
        }
    }
#endif
...
#ifdef ZCOPY_UDP_RX_DEBUG
                    QCLI_Printf("%d Receiving %u bytes from %s:%d. total=%llu pkt=0x%p\n",
                            i, received,
                            inet_ntop(family, sin_addr, ip_str, sizeof(ip_str)),
                            ntohs(from->sa_port),
                            p_tCxt->bytes, pkt);
...
int bench_common_wait_for_response(THROUGHPUT_CXT *p_tCxt, struct sockaddr *to, uint32_t tolen, uint32_t cur_packet_number)
{
    ...
    if (p_tCxt->test_type == TX && p_tCxt->protocol == UDP)
    {
        ...
        if ((p_tCxt->sock_local = qapi_socket(family, SOCK_DGRAM, 0)) == A_ERROR)
        {
            QCLI_Printf("%s: Socket creation error\n", __func__);
            goto ERROR_1;
        }
        ...
        if (qapi_bind(p_tCxt->sock_local, addr, addrlen) != QAPI_OK)
        {
            QCLI_Printf("%s: Socket bind error\n", __func__);
            goto ERROR_2;
        }
    }
    ...
    if (p_tCxt->is_iperf)
    {
        ...
        */
        ...
        while (retry_counter < 10)
        {
            ...
            if (sent_bytes < 0) 
            {
                QCLI_Printf("UDP send terminate packet error %d , retry %d \r\n", sent_bytes, retry_counter);
    ...
    while (retry_counter < 10)
    {
        ...
        //QCLI_Printf("%d sent EOT_PACKET %d bytes\n", retry_counter, sent_bytes);
        ...
        if ((conn_sock = qapi_select(&rset, NULL, NULL, 200)) > 0)
        {
            ...
            {
                QCLI_Printf("%d received %u-byte statistics\n", retry_counter, received);
                error = QAPI_OK;
            ...
            {
                QCLI_Printf("%d Did not receive response: %d\n", retry_counter, received);
                retry_counter++;
            }
        }
        else
        {
            //QCLI_Printf("%d conn_sock=%d ms=%u\n", retry_counter, conn_sock, app_get_time(NULL));
            retry_counter++;
        }
    } /* while(retry_counter) */
    }
...
void bench_udp_tx(THROUGHPUT_CXT *p_tCxt)
{
    ...
    int iperf_udp_adj= 0; 
    uint32_t iperf_last_pkt_utime = 0;
    uint32_t iperf_curr_pkt_utime = 0;    
    uint32_t iperf_display_interval = 0;
    uint32_t iperf_display_last = 0;
    uint32_t iperf_display_next = 0;
    ...
    {
        QCLI_Printf("------------------------------------------------------------\n");        
        QCLI_Printf("Client connecting to %s, UDP port %d\n",ip_str, p_tCxt->params.tx_params.port);
        QCLI_Printf("------------------------------------------------------------\n");
...
//        QCLI_Printf("Delay in microseconds: %u\n", p_tCxt->params.tx_params.interval_us);
    ...
    {
        QCLI_Printf("Socket creation failed\n");
        goto ERROR_1;
    }
	...
	if (p_tCxt->params.tx_params.source_ipv4_addr != 0) {
		if (qapi_bind(p_tCxt->sock_peer, (struct sockaddr*)&src_sin, sizeof(src_sin)) == A_ERROR) {
			QCLI_Printf("Socket bind failed\n");
			goto ERROR_2;
		}
	}
    ...
    if (p_tCxt->protocol == SSL && p_tCxt->test_type == TX)
    {
        ...
        if(packet_size > dtls_data_mtu) {
            ...
            QCLI_Printf("Reducing packet size to %d to avoid IP fragmentation \n", dtls_data_mtu);
        }
    }
    ...
    if (p_tCxt->is_iperf)
    {
          ...
          iperf_udp_delay_target = (int) ( p_tCxt->params.tx_params.packet_size* ((1000*1000 * 8) 
    ...
    while ( !is_test_done )
    {
        ...
        if ( p_tCxt->buffer == NULL )
        {
            ...
            if (p_tCxt->is_iperf)
            {
                time_struct_t iperf_curr_time;
                app_get_time(&iperf_curr_time);
                iperf_last_pkt_utime = (iperf_curr_time.seconds * 1000 * 1000) +( iperf_curr_time.milliseconds * 1000);
            }
        }
        else
		{
        ...
        do
        {
            ...
            if ((p_tCxt->is_iperf) && iperf_udp_delay_target)
            {
                time_struct_t iperf_curr_time;
                app_get_time(&iperf_curr_time);
                uint32_t cur_utime = iperf_curr_time.seconds * 1000 * 1000 + iperf_curr_time.milliseconds * 1000;            
                iperf_curr_pkt_utime = cur_utime;
...
*/
            if (p_tCxt->is_iperf &&(iperf_udp_delay/1000 > 0))
            {
                qapi_Task_Delay(iperf_udp_delay/1000);
            }   
    ...
    if (!p_tCxt->is_iperf)
    {
        if (result != QAPI_OK)
        {
            QCLI_Printf("%s Transmit test failed, did not receive Ack from Peer\n", (p_tCxt->protocol == SSL && p_tCxt->test_type == TX) ? "SSL" : "UDP");
        }
    }
