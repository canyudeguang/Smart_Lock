...
*/
void Display_Function_Success(char *Function_Name)
{
...
*/
void Display_Function_Error(char *Function_Name, qapi_Status_t Result)
{
   if(Function_Name != NULL)
   {
      LOG_DEBUG("%s returned %d \r\n", Function_Name, Result);
   }
}
...
/**
   ...
   @param Buffer      is a pointer to the buffer containing the data to be
                      displayed.
*/
void Dump_Data(const char *Prefix, uint16_t Length, const uint8_t *Buffer)
{
   uint16_t DataOffset;
   uint32_t LineOffset;
   uint32_t Index;
