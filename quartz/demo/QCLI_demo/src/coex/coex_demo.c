...
const QCLI_Command_t Coex_CMD_List[] =
{
   ...
   {cmd_SetEXTPriority,            false, "SetEXTPriority",           "[LowRequest] [HighRequest] [LowActive] [HighActive]",                                                                                                                                                                                                                                                                       "Sets EXT priorities for advanced configuration."},
    ...
    {cmd_configureWlanCoex,         false, "EnableWlanCoex",           "<WLAN coex enablement. enable|disable> <coex mode. 3w|pta|epta> <num coex priority levels. 2|4> <profile type. 1-11> <antenna config. 1-3>",                                                                                                                                                                                    "Enable and configure coex or disable coex. No subsequent parameters if 'coex enablement' is 'disable'"},
...
{
    if (!Parameter_List[index].Integer_Is_Valid || (Parameter_List[index].Integer_Value < 1 || Parameter_List[index].Integer_Value > 13)) {
        QCLI_Printf(Coex_Demo_Context.QCLI_Handle,
...
cmd_configureWlanCoex(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
    ...
    if (APP_STRICMP(Parameter_List[0].String_Value, "enable") == 0)
    {
        ...
        if (!Parameter_List[2].Integer_Is_Valid || !(Parameter_List[2].Integer_Value == 2 || Parameter_List[2].Integer_Value == 4))
        {
            QCLI_Printf(Coex_Demo_Context.QCLI_Handle,
                        "Error: Incorrect parameter value or format for number of priority levels. Provide 2 or 4.\r\n");
            return QCLI_STATUS_ERROR_E;
        }
        WLAN_CoexConfig.coex_Num_Priority_Levels = ((Parameter_List[2].Integer_Value == 2) ? QAPI_WLAN_COEX_NUM_PRIORITY_LEVELS_TWO_E :
                                                    QAPI_WLAN_COEX_NUM_PRIORITY_LEVELS_FOUR_E);
        ...
        if (!Parameter_List[4].Integer_Is_Valid || (Parameter_List[4].Integer_Value < 1 || Parameter_List[4].Integer_Value > 3)) {
            QCLI_Printf(Coex_Demo_Context.QCLI_Handle, "Specify a valid antenna config value:\n"
                        "\t1  - Single, Shared Antenna\n"
                        "\t2  - Dual Antenna, Low Isolation\n"
                        "\t3  - Dual Antenna, High Isolation\n\n");
            return QCLI_STATUS_ERROR_E;
        }
        WLAN_CoexConfig.coex_Antenna_Config = Parameter_List[4].Integer_Value;
    }
...
cmd_GetWlanCoexStats(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
    ...
    if (0 != qapi_Get_WLAN_Coex_Stats(WLAN_CoexStats))
    {
        ...
        QCLI_Printf(Coex_Demo_Context.QCLI_Handle, "---\nGENERAL COEX STATS\nhighRatePktCnt = \t\t%d\n"
                    ...
                    WLAN_CoexStats->coex_Stats_Data.generalStats.optModeTransitionCnt);
