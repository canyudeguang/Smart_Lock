...
fs_demo_rm_on_error:
    FS_DEMO_PRINTF("Usage: rm filename\r\n");
    return QCLI_STATUS_ERROR_E;
}
...
fs_demo_read_on_error:
    if ( QAPI_OK != status ) {
        FS_DEMO_PRINTF("Usage: read filename offset length\r\n");
        return QCLI_STATUS_ERROR_E;
    }
...
fs_demo_write_on_error:
    if ( QAPI_OK != status ) {
        FS_DEMO_PRINTF("Usage: write filename offset data_in_hex.  data_in_hex is a hexadecimal string.\r\n");
        return QCLI_STATUS_ERROR_E;
    }
...
char fs_unittest_filename[] = "/spinor/fs_unittest.bin";
