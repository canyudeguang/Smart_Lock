...
void NVMWrite_SubNetIdentityUpdate (APP_SUBNET_DATA_T *data, bool delete)
{
...
void NVMDeserialize_ModelPublicationStatus(uint8_t *buffer)
{
    ...
    QMESH_MODEL_PUBLISH_STATE_T *pub_state;
    QMESH_LABEL_UUID_T *l_publish_uuid;
    ...
    len += sizeof (pub_state->publish_addr);
    l_publish_uuid = (QMESH_LABEL_UUID_T *)malloc (sizeof(QMESH_LABEL_UUID_T));

    if (NULL == l_publish_uuid)
    {
        return;
    }

    memcpy (l_publish_uuid, buffer+len, QMESH_UUID_SIZE_OCTETS);
    pub_state->publish_uuid = l_publish_uuid;
    len += QMESH_UUID_SIZE_OCTETS;
...
QMESH_RESULT_T NVMAddKeys ()
{
    ...
    /* Initializing provisioning state */
