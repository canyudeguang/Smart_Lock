...
#define		CHK_POINT_GPIO_PIN  14

uint32  chk_pt_gpio_pin = CHK_POINT_GPIO_PIN;
...
void init_BLE()
{
	d_InitializeBluetooth(0, NULL);
	d_RegisterAIOS(0, NULL);
	
	QCLI_Parameter_t param[1];
	param[0].Integer_Value = 1;
	param[0].Integer_Is_Valid = true;
	d_AdvertiseLE(1, param);
...
static void QCLI_Thread(void *Param)
{
	...
	QCLI_Process_Input_Data(1, "6");
	Process_Command();
	clear_buffer();

	QCLI_Process_Input_Data(20, "4 5000 500 2000 1 20");
	Process_Command();
	clear_buffer();
	#endif
	...
	zigbee_printf("enter zigbee-----------------");
    QCLI_Process_Input_Data(2, "14");
    Process_Command();
    clear_buffer();
	//qurt_thread_sleep(300);
//OK
//	my_Initialize();
	cmd_ZB_Initialize(0, NULL);
	
	#endif
	#if 0
	zigbee_printf("init zigbee-----------------");
	...
	zigbee_printf("enter ZCL-----------------");
    QCLI_Process_Input_Data(2, "26");
    Process_Command();
    clear_buffer();
	//qurt_thread_sleep(300);
	...
	zigbee_printf("create point-----------------");
    QCLI_Process_Input_Data(5, "6 1 6");
    Process_Command();
    clear_buffer();
	//qurt_thread_sleep(300);

	
	zigbee_printf("back to parent dir-----------------");
    QCLI_Process_Input_Data(1, "2");
    Process_Command();
    clear_buffer();
	//qurt_thread_sleep(300);
	
	
	zigbee_printf("form network-----------------");
    QCLI_Process_Input_Data(3, "9 1");
    Process_Command();
    clear_buffer();
	qurt_thread_sleep(300);

	
	zigbee_printf("open permit-----------------");
    QCLI_Process_Input_Data(6, "14 255");
    Process_Command();
    clear_buffer();
	qurt_thread_sleep(300);
    ...
    while(true)
    {
        if(flag == 1)
            break;
		zigbee_printf("wait for connect-----------------");
        qurt_thread_sleep(300);
    }

	zigbee_printf("enter OnOff-----------------");
    QCLI_Process_Input_Data(5, "OnOff");
    Process_Command();
    clear_buffer();
