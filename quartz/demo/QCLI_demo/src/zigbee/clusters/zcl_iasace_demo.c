...
/* Structure representing the ZigBee IAS cie demo context information. */
...
static QCLI_Command_Status_t cmd_ZCL_IASACE_SetPanelStatus(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List);
...
const QCLI_Command_t ZigBee_IASACE_CMD_List[] =
{
   ...
   {cmd_ZCL_IASACE_SetPanelStatus,       false,  "SetPanelStatus",      "[ServerEndpoint][PanelStatus][SecondsRemaining][AudibleNotify][AlarmStatus]", "Sets panel status on the IAS ACE server."},
...
static QCLI_Command_Status_t cmd_ZCL_IASACE_SetClientArmCode(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   {
      strlcpy((char *)(ZigBee_IASACE_Demo_Context.ArmCode), Parameter_List[0].String_Value, sizeof(ZigBee_IASACE_Demo_Context.ArmCode));
...
/**
   @brief Executes the "SetArmCode" command to set the arm/disarm code
          on the IAS cie server or IAS cie client.
...
/**
   @brief Executes the "AddZone" command to add a zone table entry to
          the zone table on the IAS cie server.
...
static QCLI_Command_Status_t cmd_ZCL_IASACE_AddZone(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   if(GetZigBeeHandle() != NULL)
   {
...

      {
         ...
         Request.ZoneLabel   = (uint8_t *)(Parameter_List[4].String_Value);
...
/**
   @brief Executes the "DeleteZone" command to delete a zone table
          entry from the zone table on the IAS cie server..
   ...
   if(GetZigBeeHandle() != NULL)
   {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[0]), QAPI_ZB_APS_MIN_ENDPOINT, QAPI_ZB_APS_MAX_ENDPOINT)))
      {
         ...
         if(ClusterInfo != NULL)
         {
            ...
            {
               Result = qapi_ZB_CL_IASACE_Server_Find_Zone_By_ID(ClusterInfo->Handle, (uint8_t)Index, &ZoneInfo);
               if(Result == QAPI_OK)
               {
                  if(!ZoneFound)
                  {
                     /* Display the header only after a zone is found. */
                     QCLI_Printf(ZigBee_IASACE_Demo_Context.QCLI_Handle, " ID  | Address          | Type   | Label\n");
                     QCLI_Printf(ZigBee_IASACE_Demo_Context.QCLI_Handle, "-----+------------------+--------+-------\n");
                     ZoneFound = true;
                  }

                  QCLI_Printf(ZigBee_IASACE_Demo_Context.QCLI_Handle, " %3d | %08X%08X | 0x%04X | %s\n", ZoneInfo.ZoneID, (uint32_t)(ZoneInfo.ZoneAddress >> 32), (uint32_t)(ZoneInfo.ZoneAddress), ZoneInfo.ZoneType, ZoneInfo.ZoneLabel);
               }
            }
...
static QCLI_Command_Status_t cmd_ZCL_IASACE_SetPanelStatus(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   QCLI_Command_Status_t                  Ret_Val;
   ZCL_Demo_Cluster_Info_t               *ClusterInfo;
   qapi_ZB_CL_IASACE_Panel_Status_Info_t  Request;
   ...
   if(GetZigBeeHandle() != NULL)
   {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[4]), QAPI_ZB_CL_IASACE_ALARM_STATUS_NO_ALARM_E, QAPI_ZB_CL_IASACE_ALARM_STATUS_EMERGENCY_PANIC_E)))
      {
         ...
         if(ClusterInfo != NULL)
         {
            ...
            Request.PanelStatus           = (qapi_ZB_CL_IASACE_Panel_Status_t)(Parameter_List[1].Integer_Value);
            Request.SecondsRemaining      = (uint8_t)(Parameter_List[2].Integer_Value);
            Request.AudibleNotification   = (uint8_t)(Parameter_List[3].Integer_Value);
            Request.AlarmStatus           = (qapi_ZB_CL_IASACE_Alarm_Status_t)(Parameter_List[4].Integer_Value);

            Result = qapi_ZB_CL_IASACE_Server_Set_Panel_Status(ClusterInfo->Handle, &Request);
            if(Result == QAPI_OK)
            {
