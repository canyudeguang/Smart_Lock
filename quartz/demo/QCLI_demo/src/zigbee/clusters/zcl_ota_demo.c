...
/* Function prototypes. */
static QCLI_Command_Status_t cmd_ZB_OTA_DiscoverServer(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List);
...
static QCLI_Command_Status_t cmd_ZB_OTA_NotifyImage(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List);
...
const QCLI_Command_t ZigBee_OTA_CMD_List[] =
{
   ...
   {cmd_ZB_OTA_DiscoverServer, false, "DiscoverServer", "[DstAddr][ClientEndpoint]",                               "Discovers an OTA server."},
   {cmd_ZB_OTA_QueryImage,     false, "QueryImage",     "[ClientEndpoint][ImageType][FileVersion]",                "Queries for a FW image."},
   {cmd_ZB_OTA_StartTransfer,  false, "StartTransfer",  "[ClientEndpoint]",                                        "Starts the FW image transfer."},
   {cmd_ZB_OTA_NotifyImage,    false, "NotifyImage",    "[DevId][ServerEndpoint][Jitter][MfgCode][Type][Version]", "Notifies a client that a new image is available"}
};
...
static QCLI_Command_Status_t cmd_ZB_OTA_NotifyImage(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   if(GetZigBeeHandle() != NULL)
   {
         ...
         (Parameter_List[5].Integer_Is_Valid))
      {
         ...
         ClusterInfo = ZCL_FindClusterByEndpoint((uint8_t)(Parameter_List[1].Integer_Value), QAPI_ZB_CL_CLUSTER_ID_OTA_UPGRADE, ZCL_DEMO_CLUSTERTYPE_SERVER);
         Jitter      = (uint8_t)(Parameter_List[2].Integer_Value);
         ImageDefinition.ManufacturerCode = (uint16_t)(Parameter_List[3].Integer_Value);
         ImageDefinition.ImageType        = (uint16_t)(Parameter_List[4].Integer_Value);
         ImageDefinition.FileVersion      = (uint32_t)(Parameter_List[5].Integer_Value);
         ...
         if(ClusterInfo != NULL)
         {
            ...
            {
               Result = qapi_ZB_CL_OTA_Image_Notify(ClusterInfo->Handle, &SendInfo, QAPI_ZB_CL_OTA_NOTIFY_TYPE_JITTER_MFG_CODE_IMAGE_TYPE_FILE_VERSION, Jitter, &ImageDefinition);

               if(Result == QAPI_OK)
               {
                  Ret_Val = QCLI_STATUS_SUCCESS_E;
                  Display_Function_Success(ZCL_OTA_QCLI_Handle, "qapi_ZB_CL_OTA_Image_Notify");
               }
               else
               {
                  Ret_Val = QCLI_STATUS_ERROR_E;
                  Display_Function_Error(ZCL_OTA_QCLI_Handle, "qapi_ZB_CL_OTA_Image_Notify", Result);
               }
            }
            else
            {
               QCLI_Printf(ZCL_OTA_QCLI_Handle, "Invalid device ID.\n");
               Ret_Val = QCLI_STATUS_ERROR_E;
            }
         }
         else
...
static void ZB_CL_OTA_Server_CB(qapi_ZB_Handle_t ZB_Handle, qapi_ZB_Cluster_t Cluster, qapi_ZB_CL_OTA_Server_Event_Data_t *Event_Data, uint32_t CB_Param)
{
   ...
   if(Event_Data)
   {
      ...
      {
         case QAPI_ZB_CL_OTA_SERVER_EVENT_TYPE_DEFAULT_RESPONSE_E:
            QCLI_Printf(ZCL_OTA_QCLI_Handle, "Default Response:\n");
            QCLI_Printf(ZCL_OTA_QCLI_Handle, "  Status:        %d\n", Event_Data->Data.Default_Response.Status);
            QCLI_Printf(ZCL_OTA_QCLI_Handle, "  CommandID:     0x%02X\n", Event_Data->Data.Default_Response.CommandId);
            QCLI_Printf(ZCL_OTA_QCLI_Handle, "  CommandStatus: %d\n", Event_Data->Data.Default_Response.CommandStatus);
            break;

         case QAPI_ZB_CL_OTA_SERVER_EVENT_TYPE_COMMAND_COMPLETE_E:
            QCLI_Printf(ZCL_OTA_QCLI_Handle, "Command Complete:\n");
            QCLI_Printf(ZCL_OTA_QCLI_Handle, "  CommandStatus: %d\n", Event_Data->Data.Command_Complete.CommandStatus);
            break;
         ...
         case QAPI_ZB_CL_OTA_SERVER_EVENT_TYPE_IMAGE_READ_E:
            ...
            if(ImgDescriptor)
            {
               ...
               if(FileOffset < ZIGBEE_OTA_DEMO_PREPEND_SIZE)
               {
                  ...
                  memscpy(DataBuf, DataLen, &((uint8_t *)&ZigBee_Prepend_Header)[FileOffset], BytesRead);
      ...
      if(Event_Data)
      {
         switch(Event_Data->Event_Type)
         {
            ...
            case QAPI_ZB_CL_OTA_CLIENT_EVENT_TYPE_RAW_WRITE_E:
               ZigBee_OTA_Demo_Context.TotalBytes += Event_Data->Data.Raw_Write.DataLength;
               *Event_Data->Data.Raw_Write.ReturnStatus = QAPI_OK;
               QCLI_Printf(ZCL_OTA_QCLI_Handle, "OTA client raw write callback received, data length %d, total %d.\n", Event_Data->Data.Raw_Write.DataLength, ZigBee_OTA_Demo_Context.TotalBytes);
               break;

            case QAPI_ZB_CL_OTA_CLIENT_EVENT_TYPE_WRITE_E:
               *Event_Data->Data.Write.ReturnStatus = QAPI_OK;
            ...
            case QAPI_ZB_CL_OTA_CLIENT_EVENT_TYPE_UPGRADE_E:
               ...
               *Event_Data->Data.Upgrade.ReturnStatus = QAPI_OK;
               break;
