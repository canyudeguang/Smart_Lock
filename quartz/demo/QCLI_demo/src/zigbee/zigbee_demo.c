...
QCLI_Command_Status_t cmd_ZB_Initialize(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List);
...
static QCLI_Command_Status_t cmd_ZB_Direct_Join(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List);
...
const QCLI_Command_t ZigBee_CMD_List[] =
{
   ...
   {cmd_ZB_Direct_Join,     false,  "Direct_Join",     "",                                                                            ""},
...
void my_Initialize()
{
	...
	{
		ZDP_Demo_StackInitialize(ZigBee_Demo_Context.ZigBee_Handle);
...
QCLI_Command_Status_t cmd_ZB_Initialize(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Initialize +1\n");

   /* Verify the ZigBee layer had not been initialized yet. */
   if(ZigBee_Demo_Context.ZigBee_Handle == NULL)
   {
	   QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Initialize +2\n");
      UsePersist = false;

      if(Parameter_Count >= 1)
      {
		  QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Initialize Parameter_Count >= 1\n");
         ...
         {
			 QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Initialize Parameter_Count >= 1 error\n");
            Ret_Val = QCLI_STATUS_USAGE_E;
         }
      }
      else
      {
		  QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Initialize Parameter_Count >= 1 else\n");
         Ret_Val = QCLI_STATUS_SUCCESS_E;
      }

      if(Ret_Val == QCLI_STATUS_SUCCESS_E)
      {
		  QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Initialize before qapi_ZB_Initialize \n");
         Result = qapi_ZB_Initialize(&(ZigBee_Demo_Context.ZigBee_Handle), ZB_Event_CB, 0);
			QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Initialize after qapi_ZB_Initialize \n");
			
         if((Result == QAPI_OK) && (ZigBee_Demo_Context.ZigBee_Handle != NULL))
         {
            if(UsePersist)
            {
				QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Initialize in  UsePersist\n");
            ...
            if(Result == QAPI_OK)
            {
				QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "before ZDP_Demo_StackInitialize.\n");
               if(ZDP_Demo_StackInitialize(ZigBee_Demo_Context.ZigBee_Handle))
               {
				   QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "after ZDP_Demo_StackInitialize.\n");
                  Result = qapi_ZB_Get_Extended_Address(ZigBee_Demo_Context.ZigBee_Handle, &Extended_Address);
                  if(Result == QAPI_OK)
                  {
                     QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "ZigBee stack initialized.\n");
            ...
            if(Ret_Val != QCLI_STATUS_SUCCESS_E)
            {
				QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "in qapi_ZB_Shutdown.\n");
...
static QCLI_Command_Status_t cmd_ZB_Form(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   if(ZigBee_Demo_Context.ZigBee_Handle != NULL)
   {
	   QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Form   +1\n");
         ...
         (Verify_Integer_Parameter(&(Parameter_List[0]), 0, 1)))
      {
		 ...
		 QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Form   +2\n");
         if(Ret_Val == QCLI_STATUS_SUCCESS_E)
         {
			...
			QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Form   before qapi_ZB_Form\n");
            Result = qapi_ZB_Form(ZigBee_Demo_Context.ZigBee_Handle, &NetworkConfig);
			QCLI_Printf(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Form   after qapi_ZB_Form\n");
...
static QCLI_Command_Status_t cmd_ZB_Direct_Join(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   QCLI_Command_Status_t Ret_Val;
   qapi_Status_t         Result;
   qapi_ZB_NLME_Direct_Join_Confirm_t ConfirmData;
   qapi_ZB_NLME_Direct_Join_Request_t RequestData;
   
   RequestData.DeviceAddr = 0xB0CE181403030CCB;
   RequestData.CapabilityInfo = 140;

   /* Verify the ZigBee layer had been initialized. */
   if(ZigBee_Demo_Context.ZigBee_Handle != NULL)
   {
         Result = qapi_ZB_NLME_Direct_Join(ZigBee_Demo_Context.ZigBee_Handle, &RequestData, &ConfirmData);

         if(Result == QAPI_OK)
         {
            Display_Function_Success(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Direct_Join");
            Ret_Val = QCLI_STATUS_SUCCESS_E;
         }
         else
         {
            Display_Function_Error(ZigBee_Demo_Context.QCLI_Handle, "cmd_ZB_Direct_Join", Result);
            Ret_Val = QCLI_STATUS_ERROR_E;
         }
