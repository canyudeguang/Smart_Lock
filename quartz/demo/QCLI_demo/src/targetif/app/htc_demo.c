...
#define HTC_RAW_APP_BUFFER_SIZE 256
...
static void htc_dump_rxbuffer(uint8 endPointId)
{
	raw_htc_buffer *busy;
	uint32 length;
	int readPtr, i;
	...
	QCLI_Printf(qcli_htcdemo_hdl, "%s Received Packet of Length %d on Endpoint %d:\n", __func__, length,endPointId );
	
	/* Packet has been completely read. Queue it with HTC */
	for (i=0; i < length; i++) 
	{
		QCLI_Printf(qcli_htcdemo_hdl, "%x ",busy->data[readPtr+i]);
	}
                                     ...
                                     QCLI_Parameter_t * parameters)
{
	...
	app_buffer = (unsigned char *) malloc(HTC_RAW_APP_BUFFER_SIZE);
	if (app_buffer == NULL) return QCLI_STATUS_ERROR_E;

	htc_init_txbuffer(app_buffer, HTC_RAW_APP_BUFFER_SIZE);
                                     ...
                                     QCLI_Parameter_t * parameters)
{
    ...
    memset (&raw_htc_read_buffer, 0, sizeof(raw_htc_read_buffer_t));
    memset (&raw_htc_write_buffer, 0, sizeof(raw_htc_read_buffer_t));    
    app_buffer = NULL;
                                     ...
                                     QCLI_Parameter_t * parameters)
{
	...
	while (!read_buffer_available[endPointId]) {
		...
		qurt_signal_wait(&raw_htc_read_queue[endPointId],
    ...
    memset(app_buffer, 0, HTC_RAW_APP_BUFFER_SIZE);
                                     ...
                                     QCLI_Parameter_t * parameters)
{
	...
	free->length = length;
	htc_host_buffer_send(htcTarget, endPointId, &free->data[writePtr], length, free);
	write_buffer_available[endPointId] = FALSE;
	qurt_mutex_unlock(&htc_raw_write_mutex[endPointId]);
