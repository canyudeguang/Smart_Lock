...
int bench_raw_tx(THROUGHPUT_CXT *p_tCxt)
{
    ...
    if (proto > 255)
    {
        QCLI_Printf(qcli_net_handle, "ERROR: Protocol is greater than 255.\n");
        return -1;
    }
    ...
    {
        QCLI_Printf(qcli_net_handle, "ERROR: Socket creation failed\n");
        return -1;
    }
    ...
    if (p_tCxt->protocol == IP_RAW_HDR && p_tCxt->test_type == TX)
    {
        ...
        {
            QCLI_Printf(qcli_net_handle, "ERROR: set IP_HDRINCL failure\n");
            goto ERROR_2;
        }
    ...
    QCLI_Printf(qcli_net_handle, "Raw Socket TX Test (IP_HDRINCL is %s)\n", (p_tCxt->protocol == IP_RAW_HDR && p_tCxt->test_type == TX) ? "on" : "off");
    QCLI_Printf(qcli_net_handle, "****************************************************************\n");
    QCLI_Printf(qcli_net_handle, "Remote IP addr: %s\n", ip_str);
    QCLI_Printf(qcli_net_handle, "Protocol: %d\n", proto);
    ...
    QCLI_Printf(qcli_net_handle, "Delay in microseconds: %d\n", p_tCxt->params.tx_params.interval_us);
    ...
    {
        QCLI_Printf(qcli_net_handle, "ERROR: Conection failed\n");
        goto ERROR_2;
    }
        ...
        do
        {
            ...
            if ( send_bytes != packet_size )
            {
                ...
                if ( (EPIPE == errno) ||
                     ...
                     (EIEIO == errno))
                {
                    QCLI_Printf(qcli_net_handle, "Exxxx: send_bytes=%d, errno=%d\n", send_bytes, errno);
                    is_test_done = 1;
                    break;
                }
                else
                {
                    if ( ENOBUFS != errno )
                    {
                        QCLI_Printf(qcli_net_handle, "Failed to qapi_send, send_bytes=%d, errno=%d\n", send_bytes, errno);
                    }
                    else
                    {
                        //QCLI_Printf(qcli_net_handle, "ENOBUFS: send_bytes=%d\n", send_bytes);
                    }
    ...
    if (result != QAPI_OK)
    {
        QCLI_Printf(qcli_net_handle, "Raw Tx test failed, did not receive Ack from Peer\n");
    }
...
int bench_raw_rx(THROUGHPUT_CXT *p_tCxt)
{
    ...
    if (p_tCxt->params.rx_params.mcEnabled)
    {
        ...
        {
            QCLI_Printf(qcli_net_handle, "ERROR: set IP_ADD_MEMBERSHIP failure\n");
            goto ERROR_2;
        }
    }
    ...
    while (!benchrx_quit) /* Main loop */
    {
        ...
        while (!benchrx_quit)   /* Receive loop */
        {
            ...
            {
                QCLI_Printf(qcli_net_handle, "ERROR: %d received= %d\n", i, received);
                break;
            }
        } /* receive_loop */

ERROR_3:
        QCLI_Printf(qcli_net_handle, "Received %llu bytes from %s\n", p_tCxt->pktStats.bytes,
