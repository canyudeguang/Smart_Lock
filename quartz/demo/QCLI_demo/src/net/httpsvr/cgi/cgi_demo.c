...
cgi_demo(void *hp, void *f, char **filetext)
{
    ...
    if (strnicmp(form->request_Line, "GET", 3) == 0)
    {
        rc = demo_get(hp, f, filetext);
    }
    else if (strnicmp(form->request_Line, "HEAD", 4) == 0)
    {
        rc = demo_head(hp, f, filetext);
    }
    else if (strnicmp(form->request_Line, "PUT", 3) == 0)
    {
        rc = demo_set(hp, f, filetext, 0);
    }
    else if (strnicmp(form->request_Line, "POST", 4) == 0)
    {
