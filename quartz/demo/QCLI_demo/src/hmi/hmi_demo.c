...
static Send_Info_List_Entry_t* GetSendInfoListEntryByMSDUHandle(uint8_t MSDUHandle);
...
static Send_Info_List_Entry_t* GetSendInfoListEntryByMSDUHandle(uint8_t MSDUHandle)
{
   ...
   /* Search for the entry in the send info list. */
   CurrentEntry = HMI_Demo_Context.Send_Info_List;
   while((CurrentEntry != NULL) && (CurrentEntry->MCPS_Data_Request.MSDUHandle != MSDUHandle))
   {
      CurrentEntry = CurrentEntry->Next_Entry;
   }
...
static QCLI_Command_Status_t HMI_Cmd_StopReceive(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   if(HMI_Demo_Context.Interface_ID != 0)
   {
         ...
         (Verify_Integer_Parameter(&(Parameter_List[0]), 1, DEVICE_LIST_SIZE)))
      {
         ...
         if(DeleteRecieveInfoListEntry(Device_Index))
         {
   ...
   if((HMI_Demo_Context.Interface_ID != 0) && (HMI_Event != NULL))
   {
      ...
      switch(HMI_Event->Event_Type)
      {
         case QAPI_HMI_EVENT_TYPE_MCPS_DATA_CONFIRM_E:
            ...
            Send_Info_List_Entry = GetSendInfoListEntryByMSDUHandle(HMI_Event->Event_Data.MCPS_Data_Confirm.MSDUHandle);
            if(Send_Info_List_Entry != NULL)
            {
               DisplayPrompt = false;
               ...
               else
               {
                  if((CurrentTicks - Receive_Info_List_Entry->Last_Display_Ticks) > (qurt_timer_convert_time_to_ticks(RECEIVE_DATA_DISPLAY_FREQUENCY * 1000, QURT_TIME_MSEC)))
                  {
                     ...
                     if(ElapsedSeconds != 0)
                     {
                        ...
                        QCLI_Printf(HMI_Demo_Context.QCLI_Handle, "Device %d: Received %d bytes / %s (%d BPS), %d errors.\n", Receive_Info_List_Entry->Device_Index + 1, Receive_Info_List_Entry->Data_Count, SecondsToString(ElapsedSeconds, sizeof(Local_Data.TempBuffer), Local_Data.TempBuffer), BytesPerSecond, Receive_Info_List_Entry->Error_Count);
                     }
