...
#
#-------------------------------------------------------------------------------
#  $Header: //components/rel/core.ioe/1.0/bsp/build/scripts/elfManipulator/include/elfFileClass.py#2 $
#  $DateTime: 2017/11/23 05:56:15 $
#  $Author: pwbldsvc $
#  $Change: 14912225 $
...
# --------  ------  ------------------------------------------------------------
# 08/16/16  mad     bug fixes: Fixed logic in getStringByAddress, readDataByAddress and 
#                   setDataByAddress based on ELF documentation
...
class elfFile:
    ...
    ##
    # @brief    Member function to get symbol information starting with the
    #           strings of the list provided. (i.e matching the strings of the list
    #           in the prefix part)
    # @param    symbolNameList      List of strings including names of symbols.
    # @returns  List of Elf32_Sym types matching symbolNameList (i.e. starting
    #           with strings in symbolNameList) if successful, 
    #           RC_ERROR otherwise.
    def getSymbolStartWithName(self, symbolNameList):
        symtab = self.getSectionByName(".symtab")
        strtab = self.getSectionByName(".strtab")
        if (symtab != const.RC_ERROR and strtab != const.RC_ERROR):
            symbolList = []
            ...
            for symbol in Elf32_SymGenerator(symtab, strtab):
                ...
                for symbolName in symbolNameList:
                    if symbol.st_nameStr.startswith(symbolName):
    ...
    def getStringByAddress(self, address):
        for section in self.sectionHeaderTable:
            if (address >= section.sh_addr and 
               ...
               ## Mad wrong logic, corrected below 
